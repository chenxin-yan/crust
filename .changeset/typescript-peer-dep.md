...
Declare an optional `typescript: "^7.0.2"` peerDependency on all packages that ship type declarations. The builder's inference performance is measured and supported against the native TypeScript 7 compiler (the same one `create-crust` scaffolds with); plain-JavaScript consumers are unaffected (`peerDependenciesMeta.optional`).
