...
- **Contexts are pull-based command dependencies**: `defineContext(name, { flags?, requires? }, setup)` returns a factory attached with `.provide(...instances)` and consumed lazily with `await ctx.use(factory)`. Context-owned flags are the only flag-propagation mechanism (`FlagDef.inherit` and `FlagSnapshot.inherit` are removed); Context-level `requires` dependencies are constructed recursively and arrive in setup via `ctx`; `.of(value)` creates test doubles; Context values are disposed via `Symbol.dispose`/`Symbol.asyncDispose` in reverse construction order.
