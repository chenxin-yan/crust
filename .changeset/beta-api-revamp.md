...
- **Contexts are pull-based command dependencies**: `defineContext(name, { flags? }, setup)` returns a factory attached with `.provide(...instances)` and consumed lazily with `await ctx.use(factory)`. Context-owned flags are the only flag-propagation mechanism (`FlagDef.inherit` and `FlagSnapshot.inherit` are removed); setup pulls its own dependencies with `await ctx.use(factory)`; `.of(value)` creates test doubles; Context values are disposed via `Symbol.dispose`/`Symbol.asyncDispose` in reverse construction order.
