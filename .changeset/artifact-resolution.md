...
`@crustjs/crust`: `crust build` defines `process.env.CRUST_BUILD` as `"1"` in every Bun and Node bundle it produces (CLI and `--bun-plugin` paths) so `resolveArtifactDir` can tell a staged bundle from source.
