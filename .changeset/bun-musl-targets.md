...
- `bun-linux-x64-baseline` and `bun-windows-x64-baseline` are replaced by `bun-linux-x64` and `bun-windows-x64`; Bun ships one x64 binary and treats the suffix as a legacy alias. Binary filenames lose the suffix (`my-cli-bun-linux-x64-baseline` → `my-cli-bun-linux-x64`), and the old target names fail with a did-you-mean hint.
