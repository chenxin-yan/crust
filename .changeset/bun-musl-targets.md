...
- `bun-linux-x64-baseline` and `bun-windows-x64-baseline` are replaced by `bun-linux-x64` and `bun-windows-x64`; Bun 1.4 ships one x64 binary and treats the suffix as a legacy alias. Binary filenames lose the suffix (`my-cli-bun-linux-x64-baseline` → `my-cli-bun-linux-x64`), and the old target names fail with a did-you-mean hint.
- `crust build` now requires Bun 1.4.0 or newer when compiling executables (`engines.bun` is `>=1.4.0`) and refuses an older `bun` on `PATH`. On Bun 1.3 the canonical x64 names select the AVX2 build, which crashes on older CPUs.
