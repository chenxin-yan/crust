...
- `visibleWidth` removed — use `stringWidth(text)` exported from `@crustjs/style`, which is ANSI-aware and CJK-aware by default. `padStart`/`padEnd`/`center` are unchanged.
