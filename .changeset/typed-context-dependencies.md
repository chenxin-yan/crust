...
- `.provide()`, `.add()`, and `.extend()` reject unsatisfied dependency graphs at the composition site; `.provide()` also rejects same-name providers whose value type does not satisfy the declared dependency.
- Extension `uses` are validated against every command path when the application prepares.
