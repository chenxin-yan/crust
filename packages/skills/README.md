...
Add `dist/skills` to the package's `files` array. Each `skills/<name>/SKILL.md` follows the [Agent Skills specification](https://agentskills.io/specification) and declares required `name` and `description` frontmatter.
...
The contributed `skill` command creates symlinks from selected agent directories to the packaged skill directories. Project links are relative, so name-based package paths remain stable across npm, pnpm, and Bun reinstalls and updates; global links are absolute. This name-based behavior was [empirically verified](../../docs/research/skill-cleanup-lifecycle.md#7-round-4-empirical-symlink-verification-npm-1119--pnpm-1120--bun-1314-linux). There is no fallback install mechanism, ownership manifest, or mutable skill store.
