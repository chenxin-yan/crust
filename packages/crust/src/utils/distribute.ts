...
function inferCommandName(baseName: string, bin: UserPackageJson["bin"]): string {
	if (!bin || isBinPath(bin)) return baseName;
...
function resolveDistributionMetadata(
	...
	const baseName = pkgJson.name.replace(/^@[^/]+\//, "");
	const commandName = inferCommandName(baseName, pkgJson.bin);
