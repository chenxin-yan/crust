...
project and use them as 1:1 replacements for `arg`/`flag`. They wrap
each raw Effect schema with `Schema.standardSchemaV1(...)` and project
the schema's `A`/`I` onto Standard Schema so handler types stay precise:
