...
const zodFixtures: VendorFixtures = {
	...
	optionalBoolSchema: () =>
		z.boolean().optional() as unknown as StandardSchema<unknown, boolean | undefined>,
	numberArgSchema: () => z.number() as unknown as StandardSchema<unknown, number>,
};
...
describe("help plugin integration with schema-backed definitions", () => {
	it.each([zodFixtures, effectFixtures])(
		"[$name] renders Crust metadata instead of schema annotations",
		(fx) => {
	...
	it.each([zodFixtures, effectFixtures])(
		"[$name] runs a command with schema-backed args and typed schema-backed flags",
		async (fx) => {
