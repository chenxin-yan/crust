...
function _extensionBoundary(broad: Extension) {
	...
	const nested = new Crust("app").extend(
		defineExtension(id).build(({ snapshot }) => {
...
function _checkedReplacementProof() {
	...
	const app = new Crust("app")
		...
		.extend(defineExtension(id).add(...commands));
	void app.run(["child"]);
	void app.run([], { flags: { root: 1 } });
	const replacements = [
		defineExtension(id).add(defineCommand("child", (c) => c.action(() => "replacement"))),
	];
...
function _contributedDemandValues() {
	...
	new Crust("app")
		...
		.extend(defineExtension(defineExtensionId("provider")).provide(number()));
...
function _replacementOrderAndFixedDynamicNames() {
	const name: string = "child";
	const open = defineExtension(id).add(defineCommand(name, (c) => c.action(() => "dynamic")));
	const root = new Crust("app").command("child", (c) => c.action(() => 42));
	const replaced = root.extend(open);
	void replaced.run(["child"]);
	const last = defineExtension(defineExtensionId("last")).add(
		defineCommand("child", (c) => c.action(() => true)),
	);
	const commands = [defineCommand(name, (c) => c.action(() => "dynamic"))];
	const broad = defineExtension(id).add(...commands);
...
function _checkedAliasReplacementProof() {
	...
	const aliases = [defineExtension(id).add(incoming)];
	...
	const replacements = [
		defineExtension(id).add(defineCommand("shared", (c) => c.action(() => true))),
	];
...
function _providersAfterPendingCommands() {
	...
	const nested = defineExtension(id).add(defineCommand("parent", (c) => c.add(child)));
...
function _conditionalDescendantDemand(condition: boolean) {
	...
	const contribution = defineExtension(defineExtensionId("conditional")).add(child);
	...
	const compatibleContribution = defineExtension(defineExtensionId("compatible")).add(compatible);
