...
// ValidateNamedFlagDefs
{
	// passes clean definitions through unchanged
	type Defs = readonly [
...
{
	// brands an alias that collides with a sibling flag name
	type Defs = readonly [
		{ name: "output"; type: "string"; aliases: ["out"] },
	...
	type Result = ValidateNamedFlagDefs<Defs>;
	type _check = Expect<
		Equal<Result[0]["FIX_ALIAS_COLLISION"], 'Alias "out" collides with another flag name or alias'>
	>;
}

{
	// brands aliases that collide across sibling flags
	type Defs = readonly [
		{ name: "verbose"; type: "boolean"; short: "v" },
...
{
	// brands both occurrences of a canonical name repeated within one call
...
{
	// keeps alias-free and aliased duplicate-name validation in parity
	...
	type _sameBrands = Expect<
		Equal<Extract<keyof AliasFree[0], `FIX_${string}`>, Extract<keyof Aliased[0], `FIX_${string}`>>
	>;
...
{
	// brands a short alias equal to its own flag name
...
{
	// brands spellings that collide with an existing flags record
...
{
	// brands an alias-free name that collides with an existing spelling
...
{
	// brands sometimes-async custom parsers (union return type)
	type Defs = readonly [
...
{
	// does not brand parsers returning any
...
{
	// brands a flag name starting with "no-"
...
{
	// opts widened definitions themselves out of compile-time checks
...
{
	// brands an alias starting with "no-"
...
{
	// brands empty flag names and aliases
...
{
	// brands the reserved "__proto__" spelling
