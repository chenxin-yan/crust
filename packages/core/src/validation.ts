...
function collectAncestorTokenCollisions(root: CommandNode): string[] {
	const rootTokens = new Map<string, CommandTokenOccurrence>();
	rootTokens.set(root.meta.name, { kind: "canonical name", path: [root.meta.name] });
	for (const alias of root.meta.aliases ?? []) {
		rootTokens.set(alias, { kind: "alias", path: [root.meta.name] });
	}

	const stack: Array<{
		command: CommandNode;
		path: string[];
		ancestorTokens: Map<string, CommandTokenOccurrence>;
	}> = [{ command: root, path: [root.meta.name], ancestorTokens: rootTokens }];
	...
	while (stack.length > 0) {
		...
		if (!item) break;
		const { command, path, ancestorTokens } = item;
		if (visited.has(command)) continue;
		visited.add(command);

		for (const [name, subCommand] of Object.entries(command.subCommands)) {
			const subPath = [...path, name];
			const ownTokens: Array<[string, CommandTokenOccurrence]> = [
				[name, { kind: "canonical name", path: subPath }],
			];

			for (const alias of subCommand.meta.aliases ?? []) {
				ownTokens.push([alias, { kind: "alias", path: subPath }]);
			}

			for (const [token, occurrence] of ownTokens) {
				const ancestor = ancestorTokens.get(token);
				if (!ancestor) continue;
				collisions.push(
					`Subcommand token "${token}" at "${pathLabel(
						occurrence.path,
					)}" collides with ancestor ${formatOccurrence(ancestor)}`,
				);
			}

			const childAncestorTokens = new Map(ancestorTokens);
			for (const [token, occurrence] of ownTokens) {
				childAncestorTokens.set(token, occurrence);
			}

			stack.push({ command: subCommand, path: subPath, ancestorTokens: childAncestorTokens });
		}
	}
...
export function validateCommandTree(root: CommandNode): void {
	...
	const ancestorCollisions = collectAncestorTokenCollisions(root);
	if (ancestorCollisions.length > 0) {
		throw new CrustError("DEFINITION", ancestorCollisions.join("\n"));
	}
}
