...
### Amendment 1 — 2026-05-09 (post-PR #119 staleness audit)

**Issue:** Several places in this PROMPT reference an outdated
`installSkillBundle` API shape. The actual API delivered by PR #119
(commit `dac902a`) is materially different from this PROMPT's assumptions:

```ts
// ACTUAL signature (packages/skills/src/types.ts:387)
interface InstallSkillBundleOptions {
  sourceDir: string | URL;
  agents: AgentTarget[];           // REQUIRED, not auto-detected
  version: string;                 // REQUIRED, explicit param
...
Key divergences from the PROMPT's assumed shape:

1. **There is no `meta: SkillMeta` parameter.** `name` and `description`
   are read from the bundle's `SKILL.md` YAML frontmatter (validated by
   `loadBundleFiles`), and `version` is passed as an explicit string
   parameter — typically wired to the consuming package's `package.json`
   `version`.
2. **`agents: AgentTarget[]` is required**, unlike `generateSkill` which
   auto-detects via `resolveGenerateAgents`. Pass `[]` for a no-op.
3. **Identical-version reinstalls report `up-to-date`** and skip the
   canonical-store rewrite, so the entry's `version` must bump whenever
   bundle contents change.

**Resolution:**

Replace the Step 1 `CustomSkillConfig` shape with the corrected fields:

```
- `name: string` — must satisfy `isValidSkillName`; must be unique within
  the array; must not collide with the main skill's name. The bundle's
  `SKILL.md` frontmatter must declare a matching `name:` field (the
  canonical name source for display). This duplicated `name` on the
  config entry is what the plugin uses for cheap collision-detection,
  status calls (`skillStatus({ name, ... })`), and uninstall lookups
  without having to read the bundle's frontmatter at plugin-setup time.
- `sourceDir: string | URL` — same three-mode resolution as TP-003's
  `installSkillBundle` (URL with `file:` protocol, absolute string path,
  or relative string resolved from the nearest `package.json` walking up
  from `process.argv[1]`). The canonical form for plugin authors is the
  bare relative string `"skills/<name>"`.
- `version: string` — required; typically wired to the consuming
  package's `package.json` `version` (e.g. via `import pkg from
  "./package.json" with { type: "json" }`). The plugin compares this
  against the on-disk recorded version to drive its auto-update flow.
- `description?: string` — optional; used by the multiselect UI when
  rendering the entry. When omitted, the plugin can lazily read it from
  `SKILL.md` frontmatter on first invocation, or fall back to `name`.
- `scope?: Scope` (default: inherits plugin's `defaultScope`)
- `installMode?: SkillInstallMode` (default: inherits plugin's `installMode`)
```

Note that the bundle author owns the `SKILL.md` frontmatter and any
optional fields it carries (`license`, `allowedTools`, `compatibility`,
`disable-model-invocation`, `instructions`). The plugin path does not
rewrite or inject these.

Update Step 1 setup-time validation bullets to reference `entry.name`
instead of `entry.meta.name` (lines ~103–105):

```
- Each `customSkills[].name` must satisfy `isValidSkillName`
- `customSkills[].name` must not collide with the main skill's name
  (derived from root command meta)
- `customSkills[].name` must be unique within the array
- `customSkills[].version` must be a non-empty string
```

Update Step 4 `autoUpdateSkills` loop call sites:

- Replace `skillStatus({ name: entry.meta.name, agents, scope })` with
  `skillStatus({ name: entry.name, agents, scope })`
- Replace `installSkillBundle({ meta: entry.meta, sourceDir: entry.sourceDir, agents: agentsToUpdate, scope, installMode })`
  with `installSkillBundle({ sourceDir: entry.sourceDir, agents: agentsToUpdate, version: entry.version, scope, installMode })`

Update the Mission paragraph reference at line 31 from
`{ meta, sourceDir, scope?, installMode? }` to
`{ name, sourceDir, version, scope?, installMode? }`.

**Optional plugin-level safety check:** at plugin setup, the plugin may
optionally read each bundle's `SKILL.md` frontmatter and assert
`frontmatter.name === entry.name`. This is a *recommended* defensive
check (catches typos cheaply) but not strictly required —
`installSkillBundle` itself enforces the frontmatter contract at install
time (validates `name:` and `description:` are present and that the
frontmatter `name` is the source of truth). Worker should choose based
on startup-cost vs. fail-fast tradeoff.

**Impact:** Plugin code now compiles against the actual API and
correctly threads the explicit `version` parameter from each
`CustomSkillConfig` entry through to `installSkillBundle`. The
`SkillKind` field on `crust.json` and `SkillConflictError.kindMismatch`
still apply as documented elsewhere in this PROMPT.
