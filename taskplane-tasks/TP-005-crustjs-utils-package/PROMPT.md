...
**Issue:** Step 6 refers to "the **private** `resolveBundleSourceDir`
function" introduced in TP-003. After PR #119 (commit `dac902a`) merged,
the function is actually **exported** from `packages/skills/src/bundle.ts:73`
(`export function resolveBundleSourceDir(sourceDir: string | URL): string`)
and is exercised by direct unit tests in `packages/skills/src/bundle.test.ts`
lines 105–149 (`describe("resolveBundleSourceDir", () => { ... })`).

The function was deliberately exported (with `@internal` JSDoc) so its
edge-case behavior could be tested without scaffolding a full bundle
install. This is documented in the PROMPT for TP-003 but contradicts
TP-005's "private" framing.

**Resolution:**

1. **Step 6 wording** — update the bundle.ts cleanup bullet to:

   ```
   - Delete the exported `resolveBundleSourceDir` function (introduced
     in TP-003 Step 4 at `packages/skills/src/bundle.ts:73`, exported
     for unit-test access)
   - Delete the private `findNearestPackageRoot` helper at line 37
     (TP-003 inlined a copy of `@crustjs/create`'s implementation;
     this duplicate goes away with the move to `@crustjs/utils`)
   - Delete the `resolveBundleSourceDir` re-export (if any) from
     `packages/skills/src/index.ts`
   - Import `resolveSourceDir` from `@crustjs/utils`
   - Replace `resolveBundleSourceDir(sourceDir)` call sites in
     `bundle.ts` (one occurrence in `installSkillBundle`'s body)
     with `resolveSourceDir(sourceDir)`
   ```

2. **Test migration** — add a new bullet under Step 6 (or extend the
   existing test-coverage bullet) noting that the seven existing
   `resolveBundleSourceDir` tests (`bundle.test.ts:105–149`: file URL,
   non-file URL rejection, absolute path, relative path against
   `process.argv[1]`, `process.argv[1]` unset, no walkable
   `package.json`) **migrate to `packages/utils/src/source.test.ts`**
   under a `describe("resolveSourceDir", () => { ... })` block. They
   are the canonical regression suite for the unified resolver and
   must pass byte-for-byte (apart from the function-name change and
   the import path).

3. **Acceptance-criteria update** — the existing acceptance bullet
   already says "`@crustjs/create` and `@crustjs/skills` no longer
   contain `findNearestPackageRoot` or `resolveTemplateDir`". Extend
   it to:

   ```
   - [ ] `@crustjs/create` and `@crustjs/skills` no longer contain
         `findNearestPackageRoot`, `resolveTemplateDir`, or
         `resolveBundleSourceDir` (verify with `grep -rn` and by
         confirming `bundle.test.ts` no longer imports the function)
   ```

**Impact:** Worker correctly identifies the export status, removes the
function from the skills public surface (a minor breaking change — but
the export was `@internal` and undocumented in the README/docs, so the
blast radius is just the test file), and migrates the existing test
coverage to its new home in utils.
