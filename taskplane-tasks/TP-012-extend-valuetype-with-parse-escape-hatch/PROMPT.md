...
### Amendment 1 — 2026-05-09 (post-PR #120 staleness audit)

**Issue 1 — `choices` validation is NOT yet wired in the parser.**

PR #120 (commit `b87e0ee`) shipped `choices?: readonly string[]` on
`StringArgDef` / `StringFlagDef` / `StringMultiFlagDef` as a **hint-only**
field. Per the JSDoc (`packages/core/src/types.ts:60-65`):

> It is **NOT** enforced at parse time in this version: passing a value
> outside `choices` does not throw, the value is still parsed as a
> string and delivered to your `run` handler.

A grep of `packages/core/src/parser.ts` and `packages/core/src/node.ts`
confirms zero references to `choices`. **This task is therefore
responsible for both adding the `parse` escape hatch AND implementing
the `choices` parse-time validation** that Step 4 assumes.

**Resolution:** Read Step 4's precedence rule ("validate against
`choices` first using the raw argv string, then call `parse` if
validation passes") as the canonical design. The implementation must:

- Add a single shared validator that checks `choices` first (raw argv
  string), then dispatches on `type`/`parse` for value coercion.
- Update the JSDoc on `StringArgDef.choices` /
  `StringFlagDef.choices` / `StringMultiFlagDef.choices` to remove the
  "NOT enforced at parse time" disclaimer once enforcement is wired.
- Add a regression test asserting that `choices`-violations produce a
  user-facing error (NOT silently delivered to `run`).
- Remove the existing TP-009 changeset language that called `choices` a
  "tooling-only hint" — add a new changeset entry for `@crustjs/core`
  noting that `choices` is now enforced at parse time. (This is a
  behavior change but pre-1.0, and the only realistic consumers today
  are the docs/examples shipped in PR #120, none of which rely on the
  hint-only behavior.)

**Issue 2 — `@crustjs/validate` public surface has shifted.**

Docs Step 6 references `@crustjs/validate` for the "when to use `parse`
vs. `@crustjs/validate`" comparison and includes example code
(`recipes.mdx`, `built-in-types.mdx`). Since this PROMPT was written:

- PR #117 (TP-013) replaced the prompts `validate` slot with a
  Standard-Schema-only API.
- PR #118 (TP-014) locked validate's root surface to exactly **8
  exported functions**: `arg`, `flag`, `commandValidator`, `field`,
  `parseValue`, `validateStandard`, `validateStandardSync`,
  `isStandardSchema`. Subpath barrels (`/runtime`, `/zod`, `/effect`)
  were removed.
- TP-018 (newly added 2026-05-07) plans to **remove `field` from
  validate's root export** (8 → 7) once it lands. TP-018 may merge
  before this task runs.

**Resolution:** During Step 6, verify the current validate surface
before writing examples:

```
# Run from repo root immediately before docs work:
cat packages/validate/src/index.ts
```

Use only the functions actually exported there. If `field` has been
moved to `@crustjs/store` (per TP-018), update the comparison example
to reflect the post-migration import. Do **not** reference removed
subpath barrels (`@crustjs/validate/runtime` etc.) or any of the
deprecated helpers TP-014 dropped (`fromSchema`, `fromValibot`,
`fromZod`, `validateInput`, `validateInputSync`, `vendor` aliases,
`legacy: true` knobs).

**Issue 3 — still blocked on TP-010 and TP-011.**

This amendment is informational — the dependency edges in the PROMPT
are correct (TP-009 ✅, TP-010 ❌, TP-011 ❌). The orchestrator will
enforce; TP-012 cannot launch until both upstream tasks are merged.
Supervisor should re-audit this PROMPT after TP-010 + TP-011 ship to
catch any further drift in the completion-plugin file paths or in
`@crustjs/utils`'s exported `BaseValueType` / `ResolvePrimitive`
shapes.

**Impact:** Worker correctly understands that PR #120 shipped
`choices` as a tooling-only hint and that wiring the parser-side
validation is part of TP-012's scope (not a TP-009 omission). Docs
examples align with the post-PR #118 / post-TP-018 validate surface
and do not regress to removed APIs.
