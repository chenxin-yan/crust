...
The OpenTUI compile smoke (`crust build --bun-plugin @opentui/solid/bun-plugin` driven in a pseudo-terminal) installs pinned packages from npm, so it is opt-in: `CRUST_TUI_SMOKE=1 bun test packages/crust/tests/tui-build.smoke.test.ts` (Linux/macOS).
