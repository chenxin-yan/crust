...
export const FEATURES: readonly Feature[] = [
	{
		...
		lang: "TypeScript",
		output: [
			run("convert.ts", "report.md --format markdown"),
			out("Converting report.md as markdown"),
		],
	},
	{
		key: "schema",
	...
	},
	{
		...
		lang: "JSON",
		output: [
			cmd("crust build --target bun-linux-x64 --target bun-darwin-arm64"),
			out(".crust/"),
			out("├── manifest.json"),
			out("├── root/bin/my-cli.js"),
			out("├── linux-x64/bin/my-cli-bun-linux-x64"),
			out("└── darwin-arm64/bin/my-cli-bun-darwin-arm64"),
		],
	},
	{
		key: "help",
		...
		output: [
			...
			out("  skills     Manage agent skill installations"),
			out(""),
			out("Options:"),
