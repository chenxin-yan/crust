...
const main = new Crust("my-cli").use(helpPlugin()).run(() => {
  /* ... */
});
...
When a command declares [`aliases`](/docs/api/types#commandmeta), they are rendered inline next to the canonical name as `name (alias1, alias2)`. The canonical name is what appears in the path, in error messages, and in `didYouMeanPlugin` suggestions.
