---
title: Help
description: Auto-generate help text for your CLI commands.
---

import { Callout } from "fumadocs-ui/components/callout";

The help extension adds `--help` / `-h` to all commands and auto-generates formatted help output.
On terminals with color support, the output is colorized automatically; in docs and non-color environments it falls back to plain text.

## Usage

```ts
import { Crust } from "@crustjs/core";
import { help } from "@crustjs/plugins";

const main = new Crust("my-cli").extend(help()).run(() => {
  /* ... */
});

await main.execute();
```

No configuration needed. The extension injects a `--help` flag into every command (including subcommands) during materialization.

## What It Does

### Flag Injection

During materialization, the extension adds the following flag to the root command and all subcommands recursively:

```ts
help: {
  type: "boolean",
  short: "h",
  description: "Show help",
}
```

### Help Display

The extension intercepts execution in two cases:

1. **`--help` is passed** — Renders help and stops execution
2. **Container command** — A command with subcommands but no `run()` handler automatically shows help

### Help Output Format

```text
my-cli - My CLI tool

USAGE:
  my-cli <command> [options]

COMMANDS:
  build               Build the project
  issue (issues, i)   Manage issues
  dev                 Start dev server

OPTIONS:
  -h, --help, --no-help          Show help
  -v, --version, --no-version    Show version number
```

When a command declares [`aliases`](/docs/api/types#commandmeta), they are rendered inline next to the canonical name as `name (alias1, alias2)`. The canonical name is what appears in the path, in error messages, and in `didYouMean()` suggestions.

The help text includes these sections (when applicable):

| Section      | When shown                                     |
| ------------ | ---------------------------------------------- |
| **Header**   | Always — `command-path - description`          |
| **USAGE**    | Always — auto-generated or custom `meta.usage` |
| **COMMANDS** | When the command has subcommands               |
| **ARGS**     | When the command has positional arguments      |
| **OPTIONS**  | When the command has flags                     |

Boolean flags are shown with both supported canonical forms:

```text
--verbose, --no-verbose
```

Default values are appended inline when the command definition includes them:

```text
[name]              Who to greet [default: "world"]
--port              Port number [default: 3000]
```

## Usage Line

The usage line is auto-generated from the command definition:

```
my-cli <command> [name] [options]
```

- `<command>` — shown for container commands (subcommands exist, no `run`)
- `<arg>` — required positional argument
- `[arg]` — optional positional argument
- `arg...` — variadic argument
- `[options]` — shown when flags exist

Override the auto-generated usage with `meta.usage`:

```ts
new Crust("deploy").meta({ usage: "deploy <env> [--dry-run]" });
// ...
```

## `renderHelp` Function

The `renderHelp` function is exported separately for generating help text programmatically:

```ts
import { renderHelp } from "@crustjs/plugins";

const helpText = renderHelp(myCommand, ["my-cli", "deploy"]);
console.log(helpText);

// path is optional — defaults to [command.meta.name]
const helpText2 = renderHelp(myCommand);
```

**Parameters:**

- `command` — The command to render help for
- `path` _(optional)_ — The command path array (e.g., `["my-cli", "deploy"]`). Defaults to `[command.meta.name]`

**Returns:** A formatted help string.

## The `--` Separator

`--help` is not recognized after the `--` separator — arguments after `--` are treated as raw args. See [the `--` separator](/docs/guide/arguments#the----separator) for details.

## Subcommand Help

Each subcommand gets its own `--help`:

```sh
my-cli --help              # Help for root command
my-cli build --help        # Help for build subcommand
my-cli create plugin --help # Help for nested subcommand
```

## Type

```ts
function help(): Extension;
function renderHelp(command: CommandNode, path?: string[]): string;
```

The help output includes the full command path:

```text
my-cli build - Build the project

USAGE:
  my-cli build [options]

OPTIONS:
  --minify, --no-minify         Disable minification
  -h, --help, --no-help         Show help
```
