...
```sh
crust build --entry src/cli.tsx --bun-plugin @opentui/solid/bun-plugin
```

Solid needs OpenTUI's JSX transform at compile time, so pass `@opentui/solid/bun-plugin` to [`crust build --bun-plugin`](/docs/guide/build-and-distribution#add-bun-bundler-plugins); the `bunfig.toml` preload covers `bun run` and build validation, not compilation. React and core apps compile with a plain `crust build`.
