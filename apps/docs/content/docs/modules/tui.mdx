...
Solid needs OpenTUI's JSX transform at compile time, so list `@opentui/solid/bun-plugin` in [`crust.bunPlugins`](/docs/guide/build-and-distribution#add-bun-bundler-plugins); the `bunfig.toml` preload covers `bun run` and build validation, not compilation. React and core apps compile with a plain `crust build`.
