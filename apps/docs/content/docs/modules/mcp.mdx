...
The snapshot carries no schema, `parse`, or schema-owned requiredness, so those definitions are advertised as optional values of their token type and validated when the command runs; `run()` accepts exactly those token values. An arg and a flag with the same name, or a definition named `raw`, cannot share the flat object and are rejected.
