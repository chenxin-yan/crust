...
| custom `parse` or Standard Schema | `{ type: "string" }`; the pipeline validates                                   |
| passthrough                       | `raw: { type: "array", items: { type: "string" } }`, delivered as `rawArgs`    |

Schema-backed definitions carry no requiredness in the snapshot, so they are advertised as optional strings and validated when the command runs. An arg and a flag with the same name, or a definition named `raw`, cannot share the flat object and are rejected.
