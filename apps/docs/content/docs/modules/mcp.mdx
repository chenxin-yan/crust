...
- Any other `completed` result (`undefined`, `Date`, `Map`, class instances, `BigInt`, `NaN`, cycles, objects holding `undefined`) and every `finished` outcome return the captured stdout as text. Exceptions during result inspection or serialization also fall back to stdout. Structured content is detached from the action's object so transport serialization cannot invoke its getters again.
