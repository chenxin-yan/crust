...
- Any other `completed` result (`undefined`, `Date`, `Map`, class instances, `BigInt`, `NaN`, cycles, objects holding `undefined`) and every `finished` outcome return the captured stdout as text. Each property of the result is read once into a detached copy that is validated and then serialized, so getters run exactly once, a `toJSON` method on the result falls back without running, and the transport never touches the action's object; a getter that throws also falls back to stdout. Capture is per object: new own keys or a changed array length detected before that object's capture finishes trigger stdout fallback without reading added values.
