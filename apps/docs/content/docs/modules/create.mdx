...
Binary files (images, fonts, compiled assets) are detected by checking the first 8 KiB for a null byte and copied as-is without interpolation.
...
- When the template URL does not use the `file:` protocol or a relative template cannot resolve from a package source.
- When the resolved template does not exist or is not a directory.
...
- `{ type: "command", cmd: string, cwd?: string }` runs a command through the platform shell (`/bin/sh` on POSIX, `cmd.exe` on Windows); `cwd` overrides the working directory.
