...
<Callout type="warn" title="Internal — do not import">
	Standard Schema introspection helpers consumed by `@crustjs/validate` and
...
Lives under `@crustjs/utils/schema` only so the `dependencies` of
`@crustjs/validate` (and `@crustjs/store`) resolve for external consumers.
Depends on `@crustjs/core` (declared as an optional `peerDependency` on
`@crustjs/utils` — callers of `resolveSourceDir` alone do not need it).
