...
<Callout type="warn" title="Pre-stable">
  **Status: pre-stable (`0.0.1`).** Public surface may change without notice until `0.1.0`. Pin to
  an exact version if depending externally — `bun add @crustjs/utils@0.0.1`.
</Callout>
...
<Callout type="warn" title="Internal — do not import">
  Standard Schema helpers consumed by `@crustjs/validate` and `@crustjs/store`. **Not part of the
  public Crust API** — may change in any release without a deprecation cycle. Use
  `@crustjs/validate` instead.
</Callout>
...
external consumers. `@standard-schema/spec` is an **optional peer dependency**
of `@crustjs/utils` rather than a hard dependency, so consumers that use only
the type primitives or `resolveSourceDir` do not install it; packages that
import this subpath (`@crustjs/validate`, `@crustjs/store`) declare
`@standard-schema/spec` directly. The subpath is core-free: it does not depend
on `@crustjs/core` and does not wrap errors in package-specific Crust error
classes.
