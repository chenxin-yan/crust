...
const config = context("config", (options: { environment: string }) => options);
...
| `context(name, setup)`             | Define a named Context factory; invoke it and attach the instance with `.provide()`          |
| `extension(name, config)`          | Define a frozen, application-wide Extension value                                            |
...
| Type              | Description                                                 |
| ----------------- | ----------------------------------------------------------- |
| `ContextFactory`  | Factory returned by `context()`                             |
