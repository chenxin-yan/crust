...
When calling `run()`, scalar JSON arguments and flags accept JSON primitives, structurally JSON-compatible object types (including named interfaces), and mutable or readonly arrays/tuples (the exported `RunInput` type itself requires `JsonValue` leaves); variadic JSON arguments and multiple JSON flags use a mutable occurrence array of JSON values. URL definitions instead accept `URL` values or mutable occurrence arrays of them. The typed API excludes `Date`, `bigint`, functions, and other non-JSON values from JSON inputs. At runtime, JSON values pass through as-is, preserving object identity without a serialization round-trip or runtime JSON guard; `undefined` omits an optional input. Untyped callers also receive their supplied values unchanged: `Date` stays a `Date`, undefined object properties survive, and `NaN`/`Infinity` are not converted to `null`.
