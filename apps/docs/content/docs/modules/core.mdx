...
JSON arguments and flags in `RunInput` accept JSON primitives, objects, and mutable or readonly arrays/tuples. Non-JSON values such as `Date`, `bigint`, and functions are rejected before the invocation reaches the parser.
