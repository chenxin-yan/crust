...
| `VALIDATION_ALIAS_DIAGNOSTICS_ENV` | Companion env var for opt-in cross-depth command token diagnostics during build-time validation                   |
...
| `AliasDiagnosticsMode`       | `"off"`, `"warn"`, or `"strict"` mode for optional cross-depth command token diagnostics   |
| `ValidateCommandTreeOptions` | Options accepted by runtime command-tree validation                                        |
