...
| Type                | Description                                                                    |
| ------------------- | ------------------------------------------------------------------------------ |
| `AnyCrust`          | Completed-app inspection, execute, and checked run; no authoring after erasure |
| `RuntimeInput<T>`   | Opaque input from `runtime(value)`, checked by the consuming operation         |
...
When calling `run()`, scalar JSON arguments and flags accept JSON primitives, structurally JSON-compatible object types (including named interfaces), and mutable or readonly arrays/tuples (the exported `RunInput` type itself requires `JsonValue` leaves); variadic JSON arguments and multiple JSON flags use a mutable occurrence array of JSON values. URL definitions instead accept `URL` values or mutable occurrence arrays of them. The typed API excludes `Date`, `bigint`, functions, and other non-JSON values from JSON inputs. Ordinary typed calls pass JSON values through as-is, preserving object identity without a serialization round-trip or runtime JSON guard; `undefined` omits an optional input. Checked calls validate the accepted value kinds without cloning JSON payloads.

Structured arguments must form a supplied prefix; defaults do not fill gaps. Required core variadics without defaults use nonempty tuples. Broad app holders and open input definitions require `runtime(input)` for invocation. `runtime(value)` also opts dynamic command names, local definitions/configs, and flag/argument collections into checked consumption; it neither validates nor copies by itself and is not an `unknown` decoder. See [Runtime input](/docs/guide/types#runtime-input) for the call forms and the migration from broad `Crust` annotations.
...
| `ExtensionFactory<Args>`       | Callable Extension constructor with a readonly `.id`; broad annotations require checked Extension attachment |
