...
const app = new Crust("deploy")
  ...
  .action(async ({ flags, ctx, stdout }) => {
    stdout(`${(await ctx.config).environment}: verbose=${flags.verbose ?? false}`);
  });
...
| `visibleSectionsFor(snapshot, consumer)` | Collect audience-filtered sections across the visible command tree                                                                                  |
