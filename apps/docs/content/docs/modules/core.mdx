...
| `sectionsFor(sections, consumer)`                                                  | Applies `only` / `except` audience filters, then merges entries with the same exact title at their first position, joining bodies with a single newline.                                        |
| `visibleSectionsFor(snapshot, consumer)`                                           | Returns section-bearing visible commands with the root at `[]`, each filtered and merged as by `sectionsFor()`; hidden subtrees are skipped and children are traversed by locale-sorted canonical name. |
