...
Scalar JSON arguments and flags in `RunInput` accept JSON primitives, objects, and mutable or readonly arrays/tuples; variadic arguments and multiple flags use a mutable occurrence array of JSON values. The typed API excludes `Date`, `bigint`, functions, and other non-JSON values. At runtime, `run()` reports unserializable values when `JSON.stringify` throws or returns `undefined`; untyped inputs otherwise follow JSON serialization semantics (for example, a `Date` serializes to a string).
