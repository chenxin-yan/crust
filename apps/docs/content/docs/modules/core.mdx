...
const config = defineContext("config", (options: { environment: string }) => options);
...
| Export                             | Description                                                                                                                    |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| [`Crust`](/docs/api/crust)         | Immutable command builder with Context `.provide()`/`.derive()`, `.handle()`, programmatic `.run()`, and terminal `.execute()` |
| [`defineCommand`](/docs/api/crust) | Create an inert reusable command definition                                                                                    |
| `defineContext(name, setup)`       | Define a named Context factory; invoke it and attach the instance with `.provide()`                                            |
...
| Type              | Description                                                 |
| ----------------- | ----------------------------------------------------------- |
| `ContextFactory`  | Factory returned by `defineContext()`                       |
