...
<Callout type="info">
Invalid color inputs (e.g. `fg("hello", "not-a-color")`) raise `TypeError` (`Invalid color input: ...`) at every depth — including `"none"` — so user bugs are not silently masked when colors are off. Empty `text` always returns `""` without consulting the parser.
</Callout>
