...
The string branch uses the standard `string & {}` autocomplete pattern so editors complete the 148 CSS [named colors](https://drafts.csswg.org/css-color/#named-colors) and the `#` hex prefix while still accepting any other string Bun's CSS parser understands (`rgb()`, `hsl()`, `lab()`, `oklch()`, …). `NamedColor` is re-exported from `@crustjs/style` if you need it in your own helpers.
...
| Export                                | Description                                                                                                                                                                                      |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `resolveColorDepth(mode, overrides?)` | Resolve the [`ColorDepth`](#colordepth) tier (`"truecolor" \| "256" \| "16" \| "none"`). Compare against `"none"` for a yes/no "is color enabled" check, or `"truecolor"` to gate 24-bit output. |
...
Scalar and callable contracts remain concise in prose: `ColorMode` is `"auto" | "always" | "never"`; `ColorDepth` is `"truecolor" | "256" | "16" | "none"`; `StyleFn` is `(text: string) => string`; `ColumnAlignment` is `"left" | "right" | "center"`. The package also exports `AnsiPair`, `ColorInput`, its strict-wrapper helper types, `StyleInstance`, `MarkdownTheme`, `PartialMarkdownTheme`, and `ThemeSlotFn`.
...
#### Hyperlink and wrapping options

<auto-type-table path="../../../../../packages/style/src/hyperlinks.ts" name="HyperlinkOptions" />

<auto-type-table path="../../../../../packages/style/src/text/wrap.ts" name="WrapOptions" />

#### List options

<auto-type-table
  path="../../../../../packages/style/src/blocks/lists.ts"
  name="UnorderedListOptions"
/>

<auto-type-table
  path="../../../../../packages/style/src/blocks/lists.ts"
  name="OrderedListOptions"
/>

<auto-type-table path="../../../../../packages/style/src/blocks/lists.ts" name="TaskListItem" />

<auto-type-table path="../../../../../packages/style/src/blocks/lists.ts" name="TaskListOptions" />
...
<auto-type-table
  path="../../../../../packages/style/src/theme/createMarkdownTheme.ts"
  name="CreateMarkdownThemeOptions"
/>
...
For a scoped override that only lasts for a single CLI run, use [`noColor()`](/docs/modules/extensions/no-color).

</Callout>

### Deterministic capability testing

Pass capability overrides to simulate terminal environments without mutating process globals:
...
it can be passed to `composeStyles` and `applyStyle` directly:
...
<Callout type="warn">
  `chain.open + text + chain.close` matches `chain(text)` byte-for-byte *only* when adjacent chain
...
#### Captured chain references

Top-level helpers and first-level properties such as `style.bold` are forwarders that re-resolve the global color mode on every call. A deeper chain such as `style.bold.red` captures the mode at access time. Capture the first-level helper and continue the chain at the call site when it must follow later global overrides:

```ts
import { setGlobalColorMode, style } from "@crustjs/style";

const bold = style.bold;
setGlobalColorMode("never");
bold.red("plain text"); // re-resolves through the captured forwarder
```
...
```ts
import { fgCode, bgCode, applyStyle, composeStyles, bold } from "@crustjs/style";

const coral = fgCode("#ff7f50");
console.log(applyStyle("coral text", coral));

const boldCoral = composeStyles(bold, fgCode([255, 127, 80]));
...
```ts
import { createStyle, linkCode, composeStyles, applyStyle, underline } from "@crustjs/style";
...
const underlinedLink = composeStyles(linkCode("https://crustjs.com"), underline);
...
<Callout type="info">
  Terminal support for OSC 8 varies by emulator. Many modern terminals support it, but there is no
  reliable cross-terminal capability probe yet, so unsupported terminals may simply show plain text
  without clickable behavior.
</Callout>
...
List helpers support indentation, custom markers, and multiline alignment:

```ts
import { orderedList, taskList, unorderedList } from "@crustjs/style";

unorderedList(["apples", "bananas"]);
...
Tables support per-column alignment, custom padding and separators, and ANSI-styled cells while preserving visible alignment.
