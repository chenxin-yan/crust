...
| `createTheme(options?)` | Create a theme with optional overrides |
| `buildDefaultTheme(style)` | Build a default theme from a style instance |
...
| `CreateThemeOptions` | Options for `createTheme` |
...
```ts
import { bold, red, italic } from "@crustjs/style";
...
```ts
import { defaultTheme, createTheme } from "@crustjs/style";
