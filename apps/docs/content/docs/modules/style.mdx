...
Use any color [`Bun.color()`](https://bun.com/docs/runtime/color) understands via 24-bit truecolor ANSI sequences — hex (3/6/8 digit), named CSS colors (`"red"`, `"rebeccapurple"`), `rgb()` / `rgba()`, `hsl()` / `hsla()`, `lab()`, numeric literals, `{ r, g, b, a? }` objects, and `[r, g, b]` / `[r, g, b, a]` arrays:
