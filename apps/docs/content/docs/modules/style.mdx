...
A single `fg` / `bg` pair powered by [`Bun.color()`](https://bun.com/docs/runtime/color). Accepts every input Bun.color understands — hex, named CSS colors, `rgb()` / `rgba()`, `hsl()` / `hsla()`, `lab()`, numeric (`0xff0000`), `{ r, g, b, a? }` objects, and `[r, g, b]` / `[r, g, b, a]` arrays:

| Export | Description |
| --- | --- |
| `fg(text, input)` | Apply a truecolor foreground from any [`ColorInput`](#colorinput) |
| `bg(text, input)` | Apply a truecolor background from any [`ColorInput`](#colorinput) |
...
Invalid inputs throw immediately: `fg` and `bg` raise `TypeError` (`Invalid color input: ...`) whenever `Bun.color()` cannot parse the input. Empty `text` always returns `""` without consulting the parser.
