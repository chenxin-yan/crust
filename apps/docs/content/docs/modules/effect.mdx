...
Pulls a plain Crust Context from inside a `handler()` program, typed by the factory's value (`FactoryValueOf<F>`) and failing with `CrustTaggedError`. Like `ctx.<name>`, it is lazy: untouched Contexts never build, and repeated pulls share one build. A factory whose name is not provided on the command path fails with Core's missing-context `CrustError`, surfaced as a `CrustDefinitionError` for `Effect.catchTag`. Presence on the path is not compile-checked.
