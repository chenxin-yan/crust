...
Both the program and Layer builds run with the invocation's `ctx.signal`: caller cancellation or Ctrl-C during `execute()` interrupts their fibers, including interruptible resource acquisition. Layer finalizers receive the interruption Exit. The adapter converts signal interruption into an `AbortError` regardless of the caller's abort reason, so the interruption row above applies even for a custom `Error` or `TimeoutError`. Cleanup runs without the aborted signal; uninterruptible acquisition still has to settle before cleanup can finish.
...
`fn` receives the same input a plain action receives: `args`, `flags`, `ctx`, `rawArgs`, `signal`, `command`, `rootCommand`, `stdout`, and `stderr`. Inference is unchanged. Both forms produce identical results, and the adapted action resolves with the program's success value.
