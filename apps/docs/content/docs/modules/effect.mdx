...
The program runs with the invocation's `ctx.signal`: when the caller aborts it (`execute({ signal })`, `run(path, input, { signal })`, or Ctrl-C during `execute()`), the fiber is interrupted, Layer finalizers receive the interruption Exit, and the interruption row above applies.
