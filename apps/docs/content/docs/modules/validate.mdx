...
`@crustjs/validate` provides schema-first validation and coercion for Crust commands, prompts, and stores. It is built on the [Standard Schema v1 spec](https://github.com/standard-schema/standard-schema) — any schema library that implements this spec works with the prompt and store adapters out of the box.
...
3. You pass these definitions to `defineCommand` and use `commandValidator()` as the `run` handler.
...
Prompt and store adapters (`promptValidator`, `parsePromptValue`, `parsePromptValueSync`, `storeValidator`, `storeValidatorSync`) are also re-exported — see [Prompt Validation](#prompt-validation) and [Store Validation](#store-validation). Zod 4 implements Standard Schema natively, so no wrapping is needed.
...
Validate store config objects with schema-driven validation. These adapters accept any Standard Schema v1 compatible schema and return functions matching the `StoreValidator<T>` contract from [`@crustjs/store`](/docs/modules/store).
...
### `storeValidator(schema)` / `storeValidatorSync`

Returns a validator function for use with `createStore`'s `validator` option. Returns `{ ok: true, value }` on success or `{ ok: false, issues }` on failure. The sync variant throws `TypeError` if the schema returns a Promise.
...
const store = createStore({
  ...
  defaults: { theme: "light" as const, verbose: false },
  validator: storeValidator(
    z.object({ theme: z.enum(["light", "dark"]), verbose: z.boolean() }),
  ),
});
```

<Callout>
  Effect schemas must be wrapped with `Schema.standardSchemaV1()` before passing to store adapters.
</Callout>
...
| `storeValidator`         | function | Convert a Standard Schema into an async store validator          |
| `storeValidatorSync`     | function | Convert a Standard Schema into a sync store validator            |
