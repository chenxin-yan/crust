...
flag(z.string().describe("Output format").transform((v) => v.toUpperCase()));
```
...
| `flag`             | Wrap a Zod schema with alias metadata                    |
...
const greet = defineEffectCommand({
  meta: { name: "greet", description: "Say hello" },
  args: [arg("name", Schema.String.annotations({ description: "Person to greet" }))],
  flags: {
    loud: flag(
      Schema.optionalWith(
...
- **Wrappers**: `Schema.UndefinedOr(...)`, `Schema.optionalWith(...)`, `Schema.transform(...)`, `Schema.TemplateLiteral`
...
| `flag`                | Wrap an Effect schema with alias metadata                   |
