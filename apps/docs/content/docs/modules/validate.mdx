...
raw values, then validates and transforms them with your schema.
...
Crust does not infer metadata from schemas. Use coercing schemas for CLI string
input, and pass descriptions through Crust options. Missing values are validated
as `undefined`, so `.optional()` and `.default()` work naturally.
...
<Callout type="info" title="Parser grammar lives in Crust options">
  Descriptions, aliases, and flag `type` are Crust metadata. For flags, `type`
  declares CLI grammar/token ownership; schemas decide requiredness, defaults,
  and transformations by validating actual values.
</Callout>

## Quick start — other Standard Schema vendors

Any library implementing the spec works the same way. Supply descriptions as
Crust metadata when you want them in help output:
