...
`CrustError("DEFINITION")` naming the failing definition. (`field()`
from [`@crustjs/store`](/docs/modules/store) throws
`CrustStoreError("DEFINITION")` in the equivalent case.)
...
`field()` lives in [`@crustjs/store`](/docs/modules/store) as of 0.3.0.
See the [store-field validation guide](/docs/modules/store#schema-driven-validation)
for the full reference — including runtime-vs-type-level behaviour of
...
`@crustjs/validate` 0.3.0 moves `field()` to [`@crustjs/store`](/docs/modules/store).
...
[`@crustjs/store`](/docs/modules/store) if you previously relied on the
type.

## Migrating from 0.1.x

`@crustjs/validate` 0.2.0 removes the deprecated subpath barrels and
trims the public surface to the (then) locked 8-function root.
...
- [`@crustjs/store`](/docs/modules/store) — config storage with its own
