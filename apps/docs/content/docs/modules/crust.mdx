...
Registered Extensions own build artifacts. `man()`, `skill()`, and `completion()` return the files shown when registered; `crust build` writes them, copies each top-level directory into `.crust/root/` and every platform package's `bin/`, and records every written path per `bin` entry under `build` in `.crust/manifest.json`. A CLI reads the directories back with [`resolveArtifactDir(name)`](/docs/api/crust#resolveartifactdirname).

With several `bin` entries, each entry's hooks run in their own temporary directory and the results are merged into `.crust/artifacts/`. Directories merge, so `man/greet.1` and `man/admin.1` coexist, but a path two entries both write (or a file in one and a directory in another) fails the build naming both commands.
