...
With several `bin` entries, each entry's hooks run in their own temporary directory and the results are merged into `.crust/artifacts/`. Directories merge, so `man/greet.1` and `man/admin.1` coexist, but a path two entries both write (or a file in one and a directory in another) fails the build naming both commands; a hook that replaces its output directory, like `skill()`, therefore cannot erase another entry's files.
