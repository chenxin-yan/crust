...
`crust build` runs the entry point in a snapshot subprocess before compiling it. It uses the lockstep `SNAPSHOT_PATH_ENV` and `BUILD_OUT_DIR_ENV` tooling constants internally; `.execute()` runs registered Extension build hooks in order, writes the refreshed Command Snapshot after all hooks finish, and exits before dispatch. Application code should not set these internal environment variables.
