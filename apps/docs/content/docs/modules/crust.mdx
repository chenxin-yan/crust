...
`crust build` can embed `PUBLIC_*` build-time constants from Bun's cwd env or from explicit `--env-file` inputs. Runtime env, Bun `.env` loading, and `PUBLIC_*` guidance are documented in [Environment Variables](/docs/guide/environment).
...
For the framework API (`cli` builder, contexts, extensions, etc.), use [`@crustjs/core`](/docs/modules/core) and [`@crustjs/plugins`](/docs/modules/plugins).
