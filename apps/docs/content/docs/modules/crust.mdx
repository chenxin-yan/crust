...
`crust build` runs the entry point in a snapshot subprocess before compiling it. It uses the lockstep `SNAPSHOT_PATH_ENV`, `BUILD_OUT_DIR_ENV`, and `BUILD_RESULT_PATH_ENV` tooling constants internally; `.execute()` writes the validated Command Snapshot, runs registered Extension build hooks in order, records which hooks ran, and exits before dispatch. Application code should not set these internal environment variables.
