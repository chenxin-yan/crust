...
`skills uninstall` lists the packaged skills currently linked at the chosen scope and removes the selected ones from every agent directory; `--all` removes them without prompting. `skills repair` relinks stale or dangling links without prompting for agents.
