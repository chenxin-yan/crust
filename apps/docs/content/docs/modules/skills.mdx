...
Add agent-facing guidance to a command's metadata. Each section is rendered as a Markdown heading and body in that command's generated documentation. The manifest builder uses Core's `visibleSectionsFor()` helper so hidden command trees are skipped and audiences are applied consistently. Sections are shared with every renderer by default; pass the `skill` (`crust:skills`) factory for agent-only guidance:
