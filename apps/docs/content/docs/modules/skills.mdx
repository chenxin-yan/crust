...
Skill names must conform to the [Agent Skills spec](https://agentskills.io/specification): 1–64 lowercase alphanumeric characters and hyphens, no leading/trailing/consecutive hyphens. `generateSkill()` validates the supplied name and throws a descriptive error if invalid.
