...
Crust copies the bundle's `SKILL.md` and supporting files verbatim — the
SKILL.md frontmatter is owned by the bundle author. The following entries
are excluded **at the bundle root only** (subdirectory dotfiles are kept):
