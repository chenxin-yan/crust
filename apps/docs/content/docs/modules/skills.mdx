...
The Extension registers `my-cli skills` (alias `skill`) with the `install`, `uninstall`, and `repair` subcommands. `skills install` is the canonical install command; bare `my-cli skills` is shorthand for it and shares the same handler and flags. Setting `command` to a custom name replaces the default command without adding the alias. It also contributes an `Agent skills` root section with `except: [skill]` (the Extension factory), so it appears in help, man pages, and any other consumer, but not in the skills it generates.
...
Install is additive only: it links or repairs the selected skills for the selected agents and never removes links. An interactive install asks once which packaged skills to install (only when more than one is packaged), then once which agents to install every selected skill for, with an overwrite confirmation for each unowned directory. Unselected skills and agents are left untouched, even when they already hold an installed link; removal happens only through `skills uninstall`.

`skills uninstall` lists the packaged skills currently linked at the chosen scope, with nothing preselected, and removes the selected ones from every agent directory; `--all` removes them without prompting and is the only non-interactive form. `skills repair` relinks stale or dangling links without prompting for agents.
