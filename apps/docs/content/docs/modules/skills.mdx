...
The Extension registers `my-cli skills` with `install`, `uninstall`, and `repair` subcommands, with `skill` retained as an alias. The root command runs `install`. Setting `command` to a custom name replaces the default command without adding the alias. It also contributes an `Agent skills` root section with `except: [skill]` (the Extension factory), so it appears in help, man pages, and any other consumer, but not in the skills it generates.
...
An interactive install asks once which packaged skills to install (only when more than one is packaged; unselected skills are left untouched), then once which agents to install every selected skill for, plus a scope prompt when neither `--scope` nor `defaultScope` is set and an overwrite confirmation for each unowned directory. Deselecting an agent with an installed link removes that link for the selected skills. It does not merely skip a new installation.
