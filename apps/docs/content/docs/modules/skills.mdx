...
The generated skill defaults to the root command's name and description; pass `name` or `description` to override them. Every output directory contains `SKILL.md`, its supporting files, and a `crust.json` recording its name, description, version, and kind (`"generated"` or `"bundle"`). The dedicated `outDir` is replaced on every build so removed skills cannot ship as stale files. The API does not install into the canonical store or any **agent directory**.
