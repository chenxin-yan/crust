...
try {
  ...
  if (kindMismatch) {
    console.error(`${outputDir}: existing ${kindMismatch.existing} blocks ${kindMismatch.attempted} install`);
...
<Callout type="info" title="`metadata.version` in SKILL.md is not read">
  ...
  `metadata.version` in their SKILL.md frontmatter. Crust intentionally
  does **not** read that field — the `version` option you pass to
  `installSkillBundle()` is the sole source of truth for `crust.json` and
  update detection. If you keep a `metadata.version` in your SKILL.md for
  spec compliance, keep it in sync with the value you pass here.
</Callout>
...
The following entries are excluded **at the bundle root only**
(subdirectory dotfiles are kept):

- `node_modules/`, `.DS_Store`
- Any dotfile at the root (e.g. `.git/`, `.editorconfig`, `.gitignore`)
- A pre-existing `crust.json` (Crust regenerates it)
