...
The Extension registers `my-cli skill` and `my-cli skill update`. It also contributes an `Agent skills` root section with `except: [skill]`, so it appears in help, man pages, and any other consumer, but not in the skills it generates.
...
`distDir` is read at runtime, relative to the built entry: `crust build` stages the `skills` artifact beside `bin/` in the root package and beside each platform binary, so `new URL("../skills", import.meta.url)` resolves from `.crust/root/bin/my-cli.js`, and compiled executables fall back to the `skills` directory next to the binary. The build hook writes generated and authored skills to `.crust/artifacts/skills` and reports those files as artifacts owned by `crust:skills`.
...
```ts
const root = resolveSkillSource(new URL("../skills", import.meta.url));
