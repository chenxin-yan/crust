...
Import from `@crustjs/skills` for the full public surface, or use the focused `agents`, `annotations`, `bundle`, `extension`, and `generate` subpaths to avoid loading unrelated modules. For example, `skill` is also exported from `@crustjs/skills/extension` and `generateSkill` from `@crustjs/skills/generate`.
...
Use `writeSkills()` during your package build to create a read-only **skill source** that versions with your CLI. It snapshots the app, renders the generated command-reference skill, copies each **authored skill** bundle, and writes one self-describing directory per skill under `outDir`:
...
The generated skill defaults to the root command's name and description; pass `name` or `description` to override them. If a generated and authored skill have the same name, `writeSkills()` throws `SkillSourceConflictError` before replacing `outDir`. Every output directory contains `SKILL.md`, its supporting files, and a `crust.json` recording its name, description, version, and kind (`"generated"` or `"bundle"`). The dedicated `outDir` is replaced on every build so removed skills cannot ship as stale files. The API does not install into the canonical store or any **agent directory**.
...
## Command-Level Sections

Skills render each command's `meta.sections` after its description as `## Title` markdown sections. The same plain-text sections are also available to other documentation renderers such as help and man pages.
...
const deploy = defineCommand(
  "deploy",
  {
    ...
    sections: [{ title: "Safety", body: "Ask for confirmation before production changes." }],
  },
  (command) =>
    command.action(() => {
      // ...
