...
binary files are not supported and will be corrupted on round-trip. The
SKILL.md frontmatter is owned by the bundle author; Crust does not rewrite
it, but it runs a lightweight `name:` consistency probe and rejects the
install if the declared `name` does not match `meta.name`. Bundles that omit
`name:` from frontmatter skip the probe.
