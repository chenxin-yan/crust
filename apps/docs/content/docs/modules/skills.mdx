...
export const app = new Crust("my-cli", { description: "My CLI" })
  .extend(
    skill({
      ...
      extras: [new URL("../../skills/deployment-guide", import.meta.url)],
    }),
  )
  .action(() => {});
```

When `extras` is non-empty, the build regenerates current command documentation and copies each authored skill into the output instead of copying an existing source. `source` remains the packaged directory used for runtime discovery and installation.
