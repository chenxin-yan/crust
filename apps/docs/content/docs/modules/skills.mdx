...
The Extension registers `my-cli skills` and `my-cli skills update`, with `skill` retained as an alias. Setting `command` to a custom name replaces the default command without adding the alias. It also contributes an `Agent skills` root section with `except: [skill]`, so it appears in help, man pages, and any other consumer, but not in the skills it generates.
