...
The `skill()` Extension owns its build output. During `crust build`, it copies an available packaged source wholesale to `<outdir>/skills`. If the source is unavailable (for example, on the first source build), it renders the generated skill from the prepared Command Snapshot. `crust build --package` includes the resulting directory in the staged root package. No `--skills` build flag is needed; `--no-validate` skips all Extension build hooks.
