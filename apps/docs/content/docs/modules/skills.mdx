...
An interactive install prompts at most twice: once for which packaged skills to install (skipped when only one is packaged; unselected skills are left untouched), then once for which agents to install every selected skill for. Deselecting an agent with an installed link removes that link for the selected skills. It does not merely skip a new installation.
