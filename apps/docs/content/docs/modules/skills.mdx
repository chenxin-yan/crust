...
`writeSkills()` generates one skill from `app` and copies any authored `extras` into an output directory named `skills`, creating it as needed and leaving unrelated files in place. It returns paths relative to that directory. An authored skill with the generated skill's name replaces the generated skill.
