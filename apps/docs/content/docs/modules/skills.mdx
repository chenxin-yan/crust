...
The Extension registers `my-cli skills` (alias `skill`), which runs `install`, plus the `install`, `uninstall`, and `repair` subcommands. Setting `command` to a custom name replaces the default command without adding the alias. It also contributes an `Agent skills` root section with `except: [skill]` (the Extension factory), so it appears in help, man pages, and any other consumer, but not in the skills it generates.
