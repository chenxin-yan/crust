...
Add agent-facing guidance to a command's metadata. Each section is rendered as a Markdown heading and body in that command's generated documentation. Sections are shared with every renderer by default; use the `skill.id` (`crust:skills`) consumer id for agent-only guidance:
...
const deploy = defineCommand(
  "deploy",
  {
    ...
    sections: [
      ...
      { title: "Agent procedure", body: "Inspect the preview output.", only: [skill.id] },
    ],
  },
