...
During argv routing, an unknown subcommand produces `COMMAND_NOT_FOUND` only when its parent has no Command Action. An action-bearing parent treats the unknown token as a positional argument, so `didYouMean()` does not suggest a command for that typo. See [subcommand routing](/docs/guide/subcommands).
