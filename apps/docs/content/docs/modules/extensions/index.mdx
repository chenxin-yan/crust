...
These factories return plain `Extension` values. Contributions are application-wide and applied in `.extend()` registration order. Colliding command, flag, alias, or provided Context names use deterministic last-write-wins behavior; `.extend()` has no compile-time collision brand. Namespace custom Extension contributions to avoid accidental overrides.
