...
The command name is configurable, so attach this dynamic command contribution with `.extend(completion(options))`.
...
const completion: ExtensionFactory<
...
With `completion()` registered, `crust build` writes all three shell files through its build hook and reports them as artifacts owned by `crust:completion`:
