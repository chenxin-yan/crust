...
| --------- | ---------------- | ---------------------------------- |
| `command` | `"completion"`   | Contributed root command name      |
| `binName` | Root `meta.name` | Binary name embedded in scripts    |
| `shells`  | All three shells | Files emitted by `--output-dir`    |
| `version` | `"0.0.0"`        | Text embedded in generated headers |

The positional `<shell>` accepts `bash`, `zsh`, or `fish`. With no `--output-dir`, the selected script is written through `ctx.stdout`. With `--output-dir <path>`, the Extension writes every configured shell using canonical filenames: `<bin>` for bash, `_<bin>` for zsh, and `<bin>.fish` for fish.
