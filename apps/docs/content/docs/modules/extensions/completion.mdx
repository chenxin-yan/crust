...
```ts
import { Crust, defineCommand, runtime } from "@crustjs/core";
...
The command name is configurable, so attach this dynamic command contribution with `.extend(runtime([completion(options)]))`.
