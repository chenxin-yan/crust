...
The Extension restores the previous environment values after the command finishes. Register it before Extensions such as `help()` that may finish in `preRun`, because later `preRun` hooks are then skipped.
