...
Prompt UI renders to **stderr** by default, keeping piped stdout clean unless you override the output stream. Every prompt accepts an `initial` option to skip interactivity, which is useful for prefilling from CLI flags or CI environments.
...
A schema parses the raw submitted string, renders its first issue inline on rejection (falling back to `"Validation failed"`), and returns its transformed output on success. Async schemas are supported. Passing both `schema` and `validate` rejects immediately, before rendering.

`initial` and non-TTY `default` are raw UI fallback values rather than semantic defaults. Crust parses them through `schema` before returning them, preserving the `Promise<Output>` contract. A rejected fallback rejects the prompt promise with `initial value rejected by schema: ...` or `default value rejected by schema: ...`, without rendering. Put semantic defaults inside the schema.
...
Yes/no boolean prompt. Left, Right, and Tab toggle the current value; `y`/`h` set it to true, while `n`/`l` set it to false. Confirm with Enter.
...
The default theme's styling follows [`@crustjs/style` capability detection](/docs/modules/style#color-control), including `process.stdout.isTTY`, not the resolved prompt output. `NO_COLOR` disables colors but not modifiers, hyperlinks, or the renderer's cursor-control sequences.
...
All prompts require an interactive input TTY unless a short-circuit value is available. Every prompt skips rendering when `initial` is provided, parsing it through `schema` when configured. In addition, `input`, `confirm`, `select`, `multiselect`, `filter`, and `multifilter` use an explicitly provided `default` when the resolved input is not a TTY. Otherwise, the prompt rejects with `NonInteractiveError`. See [Validation](#validation) for schema-backed fallback rejection.
...
This is a curated overview of the [root exports](https://github.com/chenxin-yan/crust/blob/main/packages/prompts/src/index.ts), not an exhaustive type inventory. Custom-prompt test helpers live in the separate [`@crustjs/prompts/testing` subpath](#testing-custom-prompts).
