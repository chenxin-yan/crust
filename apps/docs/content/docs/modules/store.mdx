...
For schema-driven validation, use [`field()`](/docs/modules/validate#store-field-validation)
...
Store field validators are validation-only. Schema transforms/coercions in the schema passed to `field()` are not written back into store state.

For more on the `field()` factory — including the runtime-vs-type-level
behavior of schema-derived defaults — see
[Store field validation in `@crustjs/validate`](/docs/modules/validate#store-field-validation).
