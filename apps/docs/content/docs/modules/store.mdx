...
`FieldDef` accepts core value definitions and schema-backed `{ schema }` definitions. The schema-backed branch carries the Standard Schema output type directly; the output must be JSON-compatible (`JsonValue | undefined`).
