...
[Standard Schema v1](https://standardschema.dev/) object. Crust stores raw
...
- `field(z.string().default("x"))` returns `"x"` on read when the field
  is missing from disk, but the inferred config type is `string |
undefined` (NOT narrowed).
...
fields; include it in `opts` when tooling needs primitive-type metadata.
...
| `DEFINITION` | `field()` received an invalid (non-Standard-Schema) input | `{}`                    |
...
`FieldDef` accepts typed inline definitions and schema-backed definitions returned by `field()`. Inline fields declare their metadata and validation directly; the schema-backed branch carries the Standard Schema output type.
...
<auto-type-table path="../../../../../packages/store/src/field.ts" name="FieldOptions" />
