...
On reads, schema transform output for existing values is discarded; only output for a missing value can supply a default or optional value. Mutations revalidate transformed output and reject read-unstable transforms.

Mutations also reject values changed by JSON serialization, including `NaN`, `Infinity`, `-0`, sparse arrays, and objects with `undefined` properties.
