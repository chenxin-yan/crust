...
`FieldDef` accepts both typed inline definitions and the schema-backed definitions returned by `field()`. The schema-backed branch carries the Standard Schema output type; it is not limited to the legacy `type` / `default` / `validate` shape.
