...
The built-in `field()` factory builds a complete `FieldDef` from any
Standard Schema — type, default, description, and the per-field validator
are all derived in one call:
