...
**Throws:** `CrustStoreError` with `PATH` code if `dirPath` or `name` is invalid, or `DEFINITION` if a field mixes `schema` with `default` or `validate`.
...
Reads the current state with defaults and core coercion applied, calls the updater, then validates and atomically persists the result. Unlike `read()`, the state passed to the updater has not undergone full field validation. Do not assume persisted values satisfy their declared types or validators at this point.
...
The final file replacement is atomic, but mutations are not serialized, even between overlapping calls in the same process. Concurrent updates or patches can overwrite each other. Await mutations sequentially for each file; multiple processes need external coordination.

**Throws:** `CrustStoreError` with `VALIDATION` if field validators reject, `PARSE` on malformed JSON or a non-object JSON root, `IO` on filesystem failure.

### `store.patch(partial)`

Applies a shallow partial update to the current state and persists. Only the provided keys are updated; everything else is preserved. As with `update()`, the current state has defaults and core coercion applied but has not undergone full field validation; the merged result is validated before persistence. Await mutations sequentially to avoid lost updates.
...
- **Update:** applies defaults and core coercion to the current config without full field validation, runs the updater, then validates and persists the result, including schema transformations.
- **Patch:** uses the same unvalidated current config as `update()`, shallow-merges the partial input, then validates and persists the result, including schema transformations.
...
| `PARSE`      | Malformed JSON or a non-object JSON root in the persisted file | `{ path: string }`      |
...
<Callout type="warn">
  `@crustjs/store` does **not** silently recover from malformed JSON or a non-object JSON root. In
  either case, `read()` rejects with `PARSE` instead of falling back to defaults.
</Callout>
...
- Mutation locking or serialization, including overlapping calls in one process. Await mutations sequentially; coordinate multiple processes externally.
- Sync API variants
