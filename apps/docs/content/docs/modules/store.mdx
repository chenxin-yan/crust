...
Put any [Standard Schema v1](https://standardschema.dev/) object directly on a field definition's `schema` key. The schema exclusively owns validation, transformation, defaults, and optionality.
...
The inferred store type follows the schema's exact output type (`InferOutput`). A schema default therefore narrows automatically; no separate Crust default is needed or allowed. Mixing `schema` with `default` or a hand-written `validate` callback is rejected with `CrustStoreError("DEFINITION")`.
...
| `DEFINITION` | A field mixes `schema` with `default` or `validate`     | `{}`                    |
...
`FieldDef` accepts core value definitions and schema-backed `{ schema }` definitions. The schema-backed branch carries the Standard Schema output type directly.
...
The package also exports `CreateStoreOptions`, `StoreUpdater`, `FieldsDef`, `InferStoreConfig`, `ValidationErrorDetails`, `DefinitionErrorDetails`, `StoreErrorCode`, `ValueType`, and the `CrustStoreError` class.
