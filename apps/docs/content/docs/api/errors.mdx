...
`.run(path, input, io)` throws the original error without rendering it or changing process status. Context and Command Action failures are not wrapped in `CrustError`.
