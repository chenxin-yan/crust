...
Cancellation can also start outside the action. Every invocation carries `ctx.signal`, an `AbortSignal` shared by Extension hooks, Contexts, and the Command Action. It aborts with an `AbortError` reason when the caller aborts `execute({ signal })` or `run(path, input, { signal })`, or on the first Ctrl-C under `execute({ sigint: "abort" })`. Work that honors the signal (`fetch`, `signal.throwIfAborted()`, an Effect program under `@crustjs/effect`) then fails with that `AbortError` and follows the same 130 path; `postRun` hooks and Context disposal still run.
