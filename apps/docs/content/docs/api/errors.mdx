...
| `PARSE`             | Argv parsing failure (unknown flag, type coercion)                                                  | `ParseErrorDetails \| undefined`      |
...
throw new CrustError("COMMAND_NOT_FOUND", `Unknown command "${name}".`, {
  ...
  available: ["build", "dev"],
  commandPath: ["my-cli"],
  parentCommand: parentSnapshot, // CommandSnapshot of the parent command
});
```
...
try {
  ...
  throw new CrustError("VALIDATION", "Operation failed").withCause(err);
}
```
...
In plugin middleware, you can also catch and handle errors within the middleware chain. See [Plugins](/docs/guide/plugins) for details.
