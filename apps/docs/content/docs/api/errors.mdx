...
Cancellation can also start outside the action. Every invocation carries `ctx.signal`, an `AbortSignal` shared by Extension hooks and the Command Action (Context setups do not receive it). It aborts when the caller aborts `execute({ signal })` or `run(path, input, { signal })`, or on the first Ctrl-C under `execute({ sigint: "abort" })`. Work that honors the signal (`fetch`, `signal.throwIfAborted()`, an Effect program under `@crustjs/effect`) then fails with the abort reason, and the rule above classifies it: Core's Ctrl-C abort and a bare `controller.abort()` carry an `AbortError` and take the 130 path; a caller's custom reason or a `TimeoutError` renders and exits 1. `postRun` hooks and Context disposal still run either way.
