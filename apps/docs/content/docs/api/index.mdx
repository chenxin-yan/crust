...
<Callout type="warn">
  ...
  [`app.snapshot()`](/docs/api/crust#snapshot), plus build- and render-time helpers such as
  `isListed()`, `sectionsFor()`, `visibleSectionsFor()`, and the presentation-neutral
  `buildCommandDocumentation()` model. These helpers move in lockstep with first-party tooling.
</Callout>
