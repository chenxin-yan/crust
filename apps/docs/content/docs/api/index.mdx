...
| [`defineExtension<MetaKeys>()`](/docs/guide/extensions#required-root-metadata) | Specialize an Extension definer with required root metadata keys       |
...
<Callout type="warn">
  ...
  `UsageSegment` types, the parse-only `bindInput()` and `customBindings()` helpers, and the
  `SNAPSHOT_PATH_ENV` and `BUILD_OUT_DIR_ENV` subprocess-protocol constants. These helpers move in
  lockstep with first-party tooling.
</Callout>
