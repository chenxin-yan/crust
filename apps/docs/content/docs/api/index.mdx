...
<Callout type="warn">
  ...
  `sectionsFor()`, `visibleSectionsFor()`, `formatDescription()`, `snapshotCommand()`, the
  presentation-neutral `buildCommandDocumentation()` model with its `CommandDocumentation`,
  `DocumentationArg`, `DocumentationFlag`, and `UsageSegment` types, and the `SNAPSHOT_PATH_ENV` and
  `BUILD_OUT_DIR_ENV` subprocess-protocol constants. These helpers move in lockstep with first-party
  tooling.
</Callout>
