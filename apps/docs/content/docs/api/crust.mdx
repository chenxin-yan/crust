...
Definition aliases must be non-empty, contain no whitespace, not start with `-`, differ from their command's canonical name, and not collide with sibling names or aliases. For TypeScript consumers, known empty, space- or tab-containing, dash-prefixed, and self-equal aliases fail at `defineCommand()` with `FIX_ALIAS_SHAPE`, while sibling collisions fail at `.add()` with `FIX_COMMAND_COLLISION`. Runtime validation throws `DEFINITION` for plain-JS consumers and cases hidden from the type system, including other whitespace, duplicate aliases within one definition, widened definitions, and `.as()` renaming a definition to one of its own aliases.

## Chaining semantics

Variadic builder methods accumulate across calls: `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()` preserve earlier registrations and reject duplicate names or spellings. TypeScript reports the statically known flag, argument, and sibling-command collisions described below; runtime validation throws `DEFINITION` for plain-JS consumers, dynamically built definitions, and runtime-only cases. `.action()` is set once and throws `DEFINITION` if called again.
...
Defines positional arguments; repeated calls append in call order. Pass values from `defineArg(name, def)` or inline literals. For TypeScript consumers, duplicate names fail typecheck with `FIX_DUPLICATE_ARG`; runtime validation throws `DEFINITION` for duplicates from plain-JS consumers or dynamically built definitions. A Promise-returning custom `parse` fails typecheck with `FIX_ASYNC_PARSE`, while runtime validation rejects declared `async` functions for plain-JS consumers. Only the final argument across all calls may be variadic.
...
Repeated `.flags()` calls accumulate local flags. For TypeScript consumers, statically known spelling collisions fail typecheck with `FIX_ALIAS_COLLISION`; runtime validation throws `DEFINITION` for collisions from plain-JS consumers or dynamically built definitions. Promise-returning custom `parse` functions similarly fail with `FIX_ASYNC_PARSE` because parsing consumes their result synchronously; runtime validation rejects declared `async` functions for plain-JS consumers.
...
Provide order is free: at invocation time, the instances on the resolved path are constructed topologically by their declared capability requirements, with registration order preserved among independent Contexts. A missing capability dependency throws `DEFINITION` at dispatch (and during `crust build` validation). For TypeScript consumers, dependency cycles whose members are provided with literal types fail with `FIX_CONTEXT_CYCLE`; cycles through inherited parent Contexts and dynamically built graphs remain runtime-only. Runtime validation reports every cycle as `DEFINITION`.
