...
A Context's top-level `flags` are installed as propagating flags on this builder and later-added descendants, without mutating the original definitions. Invocation constructs the cached topological order.

Values implementing `Symbol.dispose` or `Symbol.asyncDispose` are disposed in reverse construction order after success or failure. Context-owned flag collisions with existing effective flags fail typecheck with `FIX_ALIAS_COLLISION` for TypeScript consumers; collisions between instances in the same `.provide()` call remain runtime-only. A duplicate Context name, passing a factory instead of an instance, or any runtime-detected flag collision throws `DEFINITION`.
...
For TypeScript consumers, statically known sibling name and alias collisions fail at `.add()` with `FIX_COMMAND_COLLISION`; the statically checked alias shape violations listed above fail at `defineCommand()` with `FIX_ALIAS_SHAPE`. Runtime validation throws `CrustError` code `DEFINITION` for the same misuse from plain JavaScript or dynamically built definitions, and for runtime-only cases: empty names, duplicate provided Context names, required Context names not provided on the parent path, unrelated recipe returns, values that are not `defineCommand()` definitions, Extensions registered inside definitions, and `.as()` renaming a definition to one of its own aliases.
