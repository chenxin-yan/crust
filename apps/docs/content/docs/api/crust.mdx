...
The recipe does not execute when the definition is created. It executes once for each `.add()`, so keep configuration side-effect-free. TypeScript validates the definition and recipe.
...
Variadic builder methods accumulate across calls: `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()` preserve earlier registrations. TypeScript reports statically known invalid definitions and collisions. Runtime definition validation is intentionally omitted; argv input remains runtime-validated.
...
Values implementing `Symbol.dispose` or `Symbol.asyncDispose` are disposed in reverse construction order after post-run hooks. Context names and owned-flag collisions are checked by TypeScript. Missing providers and dynamic dependency cycles remain runtime `DEFINITION` errors because dependencies are discovered through `ctx.use()`.
...
An Extension's `provides` instances are installed on the root immediately and Extension commands and flags are materialized during preparation. Extension definitions are TypeScript-checked rather than runtime-validated.
...
Statically known sibling name and alias collisions fail at `.add()` with `FIX_COMMAND_COLLISION`; alias shape violations fail at `defineCommand()` with `FIX_ALIAS_SHAPE`. Core assumes TypeScript-checked definitions and does not repeat these checks during materialization.
