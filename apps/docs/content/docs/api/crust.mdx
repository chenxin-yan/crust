...
`signal` is an `AbortSignal` that aborts when the caller cancels the invocation: `execute({ signal })`, `run(path, input, { signal })`, or Ctrl-C under `execute({ sigint: "abort" })`. Its reason is an `AbortError`, so pass it to `fetch`, timers, or child processes and let `signal.throwIfAborted()` end the action with exit code 130. It never aborts on its own.
...
Optional `options.signal` cancels the invocation: aborting it aborts the `ctx.signal` seen by hooks, Contexts, and the action. Work that honors the signal ends with the abort reason, which `run()` returns as a `failed` outcome like any other escaping error. `run()` never installs a `SIGINT` handler.
...
`options.signal` lets the caller cancel the invocation; aborting it aborts `ctx.signal`. `options.sigint` decides what Ctrl-C does while the invocation runs. The default `"exit"` installs no handler, so the runtime terminates the process immediately. `"abort"` installs a one-shot `SIGINT` listener for the duration of the invocation: the first Ctrl-C aborts `ctx.signal` with an `AbortError` so pending work can stop, `postRun` hooks run, and Contexts are released; a second Ctrl-C reaches the runtime's default handling. The listener is removed when the invocation settles.
