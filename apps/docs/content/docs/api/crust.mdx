...
Each call returns a builder whose type records what was added so far. A ternary on the builder produces a union of two such types, and TypeScript cannot continue a variadic call from that union — it would silently drop the call's inference. Crust rejects it instead:
...
c.provide(db(), cache()); // ctx.db: Connection | null
```
