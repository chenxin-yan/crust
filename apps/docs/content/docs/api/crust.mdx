...
Definition aliases must be non-empty, contain no whitespace, not start with `-`, differ from their command's canonical name, and not collide with sibling names or aliases. TypeScript reports known invalid-alias violations at `defineCommand()` with `FIX_INVALID_ALIAS` and sibling collisions at `.add()` with `FIX_COMMAND_COLLISION`.
