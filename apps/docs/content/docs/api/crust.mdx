...
Variadic builder methods accumulate across calls: `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()` preserve earlier registrations. TypeScript reports statically known invalid definitions and collisions, so Core does not repeat those checks at runtime. Dynamic recipe behavior, Extension injection, Context behavior, documentation sections, and argv input retain their runtime checks.
...
`uses` declarations are checked at composition time. Dependencies may appear in either order within one `.provide()` call; across calls, provide dependencies first. Core does not repeat these definition checks at runtime.
...
An Extension's `provides` instances are installed on the root immediately and Extension commands and flags are materialized during preparation. Type-expressible Extension definitions are compile-time checked; preparation retains guards for dynamic recipe behavior and reserved `__proto__` flag injection.
...
Statically known sibling name and alias collisions fail at `.add()` with `FIX_COMMAND_COLLISION`; alias shape violations fail at `defineCommand()` with `FIX_ALIAS_SHAPE`. Core does not repeat those checks during materialization. It does retain runtime checks that each recipe returns its seeded builder and does not register nested Extensions.
