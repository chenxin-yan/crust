...
interface CommandDefinitionBuilder<Inherited, Local, A, Eff, Ctx> {
  ...
  derive(
    name: string,
    setup: (input: { ctx: Readonly<Ctx> }) => Awaitable<unknown>,
  ): CommandDefinitionBuilder;
  handle(
...
```ts
defineCommand<const R extends CommandRequirements = {}>(): CommandDefinitionFactory<R>
```

Creates an inert reusable definition. The double call lets TypeScript fix requirements before it infers the configured command's local inputs:
...
Attaches a Context produced by invoking a `defineContext()` factory. Contexts are inherited by descendants, constructed in registration order only for the resolved command path, and available through `ctx` in the Command Handler.

```ts
import { defineContext, Crust } from "@crustjs/core";

const database = defineContext("database", (options: { url: string }) => ({
  url: options.url,
...
Derives a named Context value from earlier provided or derived values on the command path. `.provide()` when the factory needs only options; `.derive()` when it needs `ctx`.

Derivations share `.provide()`'s registration-ordered construction, snapshot inheritance, duplicate-name rule, and native disposal. A throw aborts before the Command Handler and disposes values already constructed.

```ts
const app = new Crust("app")
  .provide(config({ url: "https://api.example.com" }))
  .derive("client", ({ ctx }) => createClient(ctx.config.url))
  .handle(({ ctx }) => ctx.client.close());
```
...
At runtime, empty names, duplicate sibling names, alias collisions, duplicate provided or derived Context names, unrelated callback returns, values that are not `defineCommand()` definitions, and Extensions forced into an untyped definition throw `CrustError` code `DEFINITION`.
