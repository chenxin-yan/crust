...
Commands and flags declared directly in `defineExtension()` participate in typed paths and inputs. Recursive Extension flags (the default) appear on root and descendant inputs; `recursive: false` and widened-`boolean` scopes appear only on the root. Contributions hidden behind a widened `Extension` type, or whose contribution arrays are assembled dynamically or conditionally, remain runtime-only and must be invoked through `execute({ argv })`. Statically known Extension command collisions with authored commands or other Extensions fail typecheck with `FIX_COMMAND_COLLISION`; dynamically assembled contributions keep last-write-wins at prepare time — namespace Extension command names to avoid that mismatch.
