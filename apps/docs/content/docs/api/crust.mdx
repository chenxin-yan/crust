...
Binds a typed command path into a reusable handle. `handle.run(input?, io?)` accepts exactly what [`run(path, input?, io?)`](#runpath-input-io) accepts after the path, with the same inference, validation, and `RunOutcome`. Use it to let commands call other commands with checked input, to expose a CLI subtree as a typed library function, or to hand a pre-bound invoker to a test helper.
...
The path is computed from how the CLI is running. Nothing is probed, so a missing directory is reported by the caller, not here.

| Situation                                                   | Result                                                                                                                        |
| ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Compiled executable (`bun build --compile`, `deno compile`) | `<name>` next to the executable: a platform package's `bin/<name>`, or wherever you placed the `--outfile` binary             |
...
| Snapshot preparation inside `crust build`                   | `<name>` under the build output directory (`.crust/artifacts/<name>`): what earlier Extension build hooks wrote in this build |
| Source (`bun run`, `node`, `deno run`)                      | `.crust/root/<name>` under the nearest `package.json` above `process.argv[1]`: the output of the last `crust build`           |
...
`crust build` marks the Bun and Node bundles it produces by defining `process.env.CRUST_INTERNAL_BUILD` as the literal `"1"`, so `resolveArtifactDir()` can tell a staged bundle from source; standalone Deno binaries are detected directly. `CRUST_INTERNAL_BUILD` is reserved: every `process.env.CRUST_INTERNAL_BUILD` read in your CLI is replaced at bundle time, regardless of the runtime environment.
