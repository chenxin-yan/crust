...
Structured input is a programmatic call: `run()` never reads `process.env`, so a flag's `env` fallback does not apply, and a `delimiter` never splits the supplied occurrence arrays. Only [`.execute()`](#executeoptions) argv parsing consults the environment.
