...
Attaches Contexts produced by invoking `defineContext()` factories. Contexts are inherited by descendants and constructed lazily when an action, Extension, or another Context's setup calls `ctx.use(factory)`. The return type merges every instance's owned `flags` into the builder's accumulated flag type.
...
Registers application-wide Extensions. Repeated calls accumulate in registration order; duplicate Extension ids across or within calls throw `DEFINITION`. `.extend()` exists on `Crust` roots and is intentionally absent from `CommandDefinitionBuilder`.
