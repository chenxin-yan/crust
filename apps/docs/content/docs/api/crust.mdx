...
`signal` is an `AbortSignal` that aborts when the caller cancels the invocation: `execute({ signal })`, `run(path, input, { signal })`, or Ctrl-C during `execute()`. Pass it to `fetch`, timers, or child processes and let `signal.throwIfAborted()` end the action. It never aborts on its own, and Context setups do not receive it.
