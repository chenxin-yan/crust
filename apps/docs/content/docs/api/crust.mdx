...
const auth = context("auth", () => ({ user: "Ada" }));
...
At runtime, empty names, duplicate sibling names, alias collisions, duplicate Contexts, unrelated callback returns, values that are not `defineCommand()` definitions, and Extensions forced into an untyped definition throw `CrustError` code `DEFINITION`.
