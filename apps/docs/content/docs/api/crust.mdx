...
`crust build` marks the Bun and Node bundles it produces so `resolveArtifactDir()` can tell a staged bundle from source; standalone Deno binaries are detected directly.
