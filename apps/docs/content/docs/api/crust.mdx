...
Variadic `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()` accumulate across calls. `.extend()` deduplicates `ExtensionId`s last-registration-wins. TypeScript rejects statically known invalid definitions and collisions; automatic consumption checks cover observable names, spellings, positional layout, Context availability, and destination flag relations. Chaining a variadic method onto a conditional builder union (`(cond ? c.provide(db()) : c).provide(...)`) is a compile error (TS2349) because the union cannot carry the call's inference. Keep the command shape static and make the Context value conditional instead, or write the full chain in each branch so `.action()` sees the honest union.
