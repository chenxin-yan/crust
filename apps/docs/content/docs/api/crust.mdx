...
The command name is already set via the constructor or `.sub()`, so only `description` and `usage` can be provided here.
...
The callback receives a fresh builder pre-typed with the parent's inheritable flags:
...
Register a pre-built subcommand from `.sub()`.

```ts
command(builder: Crust<any, any, any>): Crust<Inherited, Local, A>
```

See [`.sub()`](#subname) for the file-splitting pattern.
...
This is the factory method for the **file-splitting pattern** — it enables full type inference in separate files without needing `Crust<any, any, any>`.
