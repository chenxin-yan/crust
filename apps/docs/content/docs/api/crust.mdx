...
Declared dependencies (Context `use`, command `.use()`, Extension `.use()`) are checked at composition. Dependencies may appear in either order within one `.provide()` call; across calls, dependency providers must precede their consumers. Dynamic collections use the same variadic methods.
...
Cleanup registered with setup's `defer(cleanup)` and values implementing `Symbol.dispose` or `Symbol.asyncDispose` run in reverse registration order after post-run hooks, including when the action throws. Register cleanup immediately after acquiring each resource, so a later setup failure still releases it. Calling `defer` after setup has settled throws a `DEFINITION` error.

A returned disposable registers when setup succeeds, so it disposes before that setup's deferred callbacks. Returned-value registration is deduplicated by object identity within one invocation; `defer` callbacks are not. Returning a disposable and also deferring its close runs both mechanisms. Do not additionally close it or use `await using` in the action, or retain it for use after the invocation ends.

A disposable supplied through `.of()` registers on each invocation that pulls it; an unpulled value is left untouched. Use a fake without a disposer for repeated invocations, or let setup create a fresh resource each time.

Cleanup starts once every setup begun during the invocation has settled. Pulling a Context that was never built then rejects with a `DEFINITION` error (`context-during-disposal`), while promises for Contexts already built keep resolving. Await dependencies during setup, before registering a `defer` callback that needs them, so they are still live when that callback runs. Registering the callback first means those dependencies are disposed before it.

Setup receives the invocation's `signal: AbortSignal`, shared with actions and Extension hooks. Caller cancellation or the first Ctrl-C during `execute()` aborts it. Pass it to cancellable initialization, such as `fetch(url, { signal })`; work that ignores it can delay cleanup. Do not reuse the aborted signal for cleanup work that must finish.
...
An Extension's `.provide()` instances and their owned flags are installed immediately across the root and existing descendants, preserving more-specific local providers on descendants. Later-added descendants inherit them too. Extension commands and declared flags are materialized during preparation.
...
<Callout type="info">
  A later `.provide()` does not backfill an already-added definition. Extension-provided Contexts
  reach existing descendants, but a definition's `.use()` demands require those providers to be
...
Structured input is a programmatic call: `run()` never reads `process.env`, so a flag's `env: { name, delimiter? }` fallback does not apply, and neither delimiter splits the supplied occurrence arrays. Only [`.execute()`](#executeoptions) argv parsing consults the environment.
...
`execute()` resolves to the terminal exit code: `0` on success, `1` on failure, or `130` for cancellation. It still renders a failure once through Extension `onError` hooks ending in Core's renderer and sets `process.exitCode` on failure. When cleanup also fails after that failure was rendered, Core writes the cleanup failure to `stderr` as `Cleanup failed: …` without calling `onError` again, and the exit code is `1` even for a cancelled action. Any `Error` whose `name` is `"AbortError"`, including a `DOMException`, is cancellation.
