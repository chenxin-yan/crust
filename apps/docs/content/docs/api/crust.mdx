...
Registers application-wide Extensions. Repeated calls accumulate in registration order. `.extend()` exists on `Crust` roots and is intentionally absent from `CommandDefinitionBuilder`.
