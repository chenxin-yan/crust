...
Command actions pull provided Contexts with `await ctx.use(factory)`. Command metadata has no dependency declaration; `use()` infers the value from the factory and reports a missing provider when called. Context setup dependencies are declared with Context-level `requires` in `defineContext()`.
...
Within one `.provide()` call, order is free: dependencies may appear after their dependents. Runtime normalization sorts the complete Context path immediately, preserving registration order among independent Contexts. A missing capability dependency or cycle throws `DEFINITION` from `.provide()`. Cycles are enforced only at runtime; TypeScript brands do not attempt recursive cycle detection.
...
Structured values are serialized to argv and pass through the normal router, parser, schema, Context, and Command Action pipeline. The optional `raw` array is appended after a `--` separator and reaches the Command Action unchanged as `rawArgs`:
