...
`uses` declarations are checked at composition time. Dependencies may appear in either order within one `.provide()` call; across calls, provide dependencies first. Runtime guards remain for JavaScript and dynamically assembled inputs.
