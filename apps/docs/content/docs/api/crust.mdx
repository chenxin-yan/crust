...
Cleanup registered with setup's `defer(cleanup)` and values implementing `Symbol.dispose` or `Symbol.asyncDispose` run in reverse registration order after post-run hooks; a returned value registers when its setup succeeds, so it disposes before that setup's deferred callbacks. Returned-value registration is deduplicated by object identity within one invocation; `defer` callbacks are not. A disposable value supplied through `.of()` registers the same way, on each invocation that pulls it. Cleanup starts once every setup begun during the invocation has settled; from then on, pulling a Context that was never built rejects with `context-during-disposal`, while promises for Contexts already built keep resolving; whether the value is still live depends on registration order (see [Contexts](/docs/guide/contexts)). Calling `defer` after setup has settled throws a `DEFINITION` error. Context names, dependencies, and owned-flag collisions are checked by TypeScript; dynamically assembled owned-flag collisions throw a `DEFINITION` error from `.provide()`. Lazy bag access retains runtime `DEFINITION` guards for missing providers on dynamically assembled paths, dependency cycles, and lifecycle misuse.
