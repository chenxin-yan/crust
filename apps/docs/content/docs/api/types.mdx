...
`subCommands` is always initialized (defaults to `{}`), even when omitted in `CommandDef`. This allows plugins to inject subcommands via `addSubCommand` during setup.
...
Boolean toggle fields (`required`, `multiple`) only accept `true` — passing `false` is a type error.
...
- `addFlag` — inject a flag into a command's flags object. If the command already has a flag with the same name, the plugin's flag overrides it. [`crust build`](/docs/guide/build#pre-compile-validation) emits a warning when this happens.
...
Per-flag validation used by `defineCommand`. Reserves the `no-` prefix for boolean negation by rejecting flag names and aliases that start with `no-`.
