...
The build context contains the frozen root Command Snapshot. It deliberately carries no output directory: build tooling owns that tree, so a hook's only way to ship a file is to return it, and the [`BuildReport`](#buildreport) lists every shipped file. Its optional `MetaKeys` type parameter refines required snapshot metadata fields.
