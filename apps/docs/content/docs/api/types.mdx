...
- `addFlag` — inject a flag into a command's flags object.
- `addSubCommand` — inject a subcommand into a command's `subCommands` record. If the parent already has a subcommand with the same name (user-defined), the call is silently skipped — user definitions always take priority.
