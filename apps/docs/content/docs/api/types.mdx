...
`build(ctx)` generates artifacts in Extension registration order when invoked by build tooling and may be async. It may return `BuildArtifacts`; its errors propagate to the caller. See [Extensions](/docs/guide/extensions).
