...
### `ExecuteOptions` and `SigintPolicy`

<auto-type-table path="../../../../../packages/core/src/types.ts" name="ExecuteOptions" />

The optional argument of [`execute(options?)`](/docs/api/crust#executeoptions). `sigint` is a `SigintPolicy`: `"exit"` (default) leaves Ctrl-C to the runtime, `"abort"` turns the first Ctrl-C into an `AbortError` on `ctx.signal` and lets a second one terminate the process.
