...
`InferArgs` and `InferFlags` are internal type plumbing, not root exports. `.args()`, `.flags()`, `.provide()`, and `.derive()` infer these fields without explicit generic arguments.
...
`defineContext(name, setup)` always returns a factory. Invoke it even when `Options` is `void`.
...
Attach an instance with `.provide()`. Core constructs it lazily on the resolved path; use `.derive()` to create a value from earlier Contexts.
