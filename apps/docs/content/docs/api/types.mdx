...
`ParseResult<A, F>` is the syntax-parsed result before required-value and Standard Schema validation. Its `args` and `flags` use the runtime-erased `ParsedArgValue` and `ParsedFlagValue` unions, and its `rawArgs` holds tokens after `--`. `ValidatedInput<A, F>` is the validated boundary passed onward to Command Actions, with `InferArgs<A>` and `InferFlags<F>` applied.
