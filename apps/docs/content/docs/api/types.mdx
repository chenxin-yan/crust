...
Types inferred from the application definition that back [`run(path, input?, io?)`](/docs/api/crust#runpath-input-io) and the [`@crustjs/testing`](/docs/modules/testing) helpers. `CommandTree` is the compile-time tree accumulated by `.add()`; `CommandPath` enumerates every valid path through it (including the root path `[]`); `CommandContractAtPath` resolves the `CommandContract` a path selects; and `RunInput` is the structured `args`/`flags`/`raw` input for that contract, built from the pre-parse `InputArgs` and `InputFlags` value types. `RunInputArguments` and `RunArguments` are the tuple types that make `input` optional when the selected command requires nothing; `RunOutcome` is the status-discriminated value `run()` resolves to. `AnyCrust` accepts any fully-built application.

```ts
import type { CommandPath, CommandContractAtPath, RunInput } from "@crustjs/core";

type Tree = (typeof app)["_types"]["tree"];
type Path = CommandPath<Tree>; // e.g. readonly [] | readonly ["greet"]
type GreetInput = RunInput<
  CommandContractAtPath<(typeof app)["_types"]["contract"], readonly ["greet"]>
>;
```
