...
One file a build hook produces: a path relative to `outDir` and its `string` or `Uint8Array` content. Build tooling normalizes separators, rejects absolute or escaping paths, and rejects a path an earlier hook already produced.
