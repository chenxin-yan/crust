...
**Conflict policy.** Alias strings must not collide with the command's own canonical `name`, with any sibling's `name`, or with any sibling's alias. Each alias must be a non-empty string with no whitespace and must not start with `-`. Collisions throw a `CrustError("DEFINITION", …)` at registration time. Plugin-installed subcommands (via the `addSubCommand` setup action) are validated when registered too: a colliding alias is skipped with a warning, mirroring how a colliding canonical name is handled today. `prepareCommandTree()` re-runs the same check against the final tree.

The default policy is sibling-scoped because command routing resolves one level at a time. Ancestor, descendant, and cousin commands may reuse the same token unless you enable optional diagnostics with `prepareCommandTree({ aliasDiagnostics: "warn" | "strict" })` or `crust build --alias-diagnostics=warn`.
...
The command is **only hidden from listings**: it stays fully invocable by name (or alias), routing is unchanged, and it can still surface through tooling that looks up specific commands rather than enumerating the tree.
...
See [Types](/docs/guide/types) for semantics, error modes, and shell-completion behaviour.
