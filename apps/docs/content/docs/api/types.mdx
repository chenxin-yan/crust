...
`defineExtension(id)` starts an immutable fluent definition; `defineExtension<MetaKeys>(id)` also [requires root metadata](#rootmetakey). Every method returns a new frozen handle and leaves the original unchanged, and every handle is an `Extension` accepted by `.extend()`. There is no final build step.
