...
See [Typed input](/docs/guide/lifecycle#typed-input).
...
`HookDeps` defaults to `Deps`. An annotation for a metadata-requiring factory must preserve its metadata keys. See [authoring an Extension](/docs/guide/extensions#author-an-extension).
...
See [Error flow](/docs/guide/error-handling#error-flow).
