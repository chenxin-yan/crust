...
`defineFlag(name, def)` returns a named flag definition; `defineArg(name, def)` returns a named argument definition. Literal fields retain inference and provably invalid names/defaults/parsers fail typecheck. Broad names and dynamic definitions use the same helpers and are checked automatically at consumption.
...
Inputs declared with `type: "url"` accept `URL`; `Date`, `bigint`, functions, and other non-JSON values are excluded from JSON inputs. `RunInputArguments` and `RunArguments` are the tuple types that make `input` optional when the selected command requires nothing; `RunOutcome` is the status-discriminated value `run()` resolves to. `CommandHandle<Shape>` is the path-bound invoker returned by [`at(path)`](/docs/api/crust#atpath): a readonly `path` plus `run(input?, io?)` typed from `Shape`. `AnyCrust` accepts any fully-built application.
...
See [`run()` input](/docs/api/crust#runpath-input-io).
...
Core `run()` is quiet, captures output, and returns invocation failures instead of rejecting them. Narrow `status` before reading the branch-specific field. See [capture semantics](/docs/api/crust#runpath-input-io) for live sinks, output boundaries, and memory cost.
...
One file a build hook produces: a path relative to the build output directory and its `string` or `Uint8Array` content. Build tooling normalizes separators, rejects absolute or escaping paths, and rejects a path an earlier hook already produced, compared case-insensitively so the tree stays valid on case-insensitive filesystems.
