...
Internal representation of a single node in the command tree. Built by the CLI builder and exposed through `@crustjs/core/tooling` for read-only tooling integrations.
...
- `effectiveFlags` — Inherited flags merged with local flags (used by the parser and help extension)
...
**Conflict policy.** Alias strings must not collide with the command's own canonical `name`, with any sibling's `name`, or with any sibling's alias. Each alias must be a non-empty string with no whitespace and must not start with `-`. Collisions throw a `CrustError("DEFINITION", …)` at registration time. Extension-contributed subcommands are validated during materialization. `prepareCommandTree()` re-runs the same check against the final tree.

**Display contract.** The canonical `name` is always what appears in `commandPath`, error messages, and `didYouMean()` suggestions — it does not depend on which alias the user typed. Help text renders the inline form `name (alias1, alias2)`.
...
The command is **only hidden from listings**: it stays fully invocable by name (or alias), routing is unchanged, and it can still surface through tooling that looks up specific commands rather than enumerating the tree (e.g. `didYouMean()` suggestions).

**Tooling contract.** Any renderer or generator that walks `subCommands` to produce a user-facing listing should skip nodes where `meta.hidden === true`. The default `help()` extension follows this contract today; first-party generators (man pages, skill descriptors, completion) and any custom renderers should too.
...
[`parseArgs`](/docs/api/parse-args)), which keeps the raw string token for
...
`--flag value` / `--flag=value`. Schema-backed flags still include this parser
...
Type-less (raw) positional args resolve to `unknown` (or `unknown[]` when variadic). The downstream schema validator narrows them to the concrete output type.
