...
Ancestor and descendant command tokens are checked too, both at runtime and in TypeScript for statically-authored command trees. Commands in separate branches may reuse the same token because routing resolves one level at a time.
