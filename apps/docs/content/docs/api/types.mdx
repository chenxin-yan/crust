...
type FlagDef =
  ...
  | RawSingleFlagDef | RawMultiFlagDef;                       // schema-backed
```

The `Raw*` variants omit `type` and let a Standard Schema validate the raw
parser output (`true` / `false` / `string` for single flags, arrays for
`multiple: true`). They are the default shape produced by
[`flag()`](/docs/modules/validate#quick-start--zod) in `@crustjs/validate`.
