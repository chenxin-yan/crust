...
`build(ctx)` runs in Extension registration order when invoked by build tooling and may be async. It returns `BuildArtifacts`, the files for the tooling to write; its errors propagate to the caller. See [Extensions](/docs/guide/extensions).
