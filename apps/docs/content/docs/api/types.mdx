...
`defineContext(name, setup)` and `defineContext(name, config, setup)` always return a factory. Invoke it even when `Options` is `void`. `contextName` lets `ctx.use()` and Context dependency arrays identify the factory; `.of(value)` produces an instance returning the precomputed value with dependencies skipped while retaining its owned flags. `AnyContextFactory` is the shared factory constraint.
...
`ContextConfig.flags` declares propagating flags installed when an instance is provided; `requires` lists Context capabilities needed by this Context's setup. Commands do not declare requirements; they pull a typed value with `ctx.use(factory)`. `ContextSetup` receives the factory argument, validated values for its owned flags only, the declared Context values, and the invocation's `stdout` and `stderr` callbacks. See [Contexts](/docs/guide/contexts) for construction order and collision rules.
...
Attach an instance with `.provide()`. Core constructs it lazily on the first `ctx.use(factory)`, recursively resolving declared Context dependencies first and memoizing the value for the invocation.
...
The map shape used for Context setup dependency values. Command actions and Extension hooks receive the resolver's generic `use(factory)` function, which returns `Promise<FactoryValueOf<typeof factory>>`.
