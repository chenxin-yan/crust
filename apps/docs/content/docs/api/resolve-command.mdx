...
```ts
function resolveCommand(command: CommandNode, argv: string[]): CommandRoute;
```
...
interface CommandNotFoundErrorDetails {
  ...
  parentCommand: CommandSnapshot; // Serializable snapshot of the parent command
}
```
...
const app = new Crust("my-cli").command("build", (cmd) => cmd.run(() => {}));
...
```ts
const app = new Crust("my-cli").command("issue", (cmd) =>
