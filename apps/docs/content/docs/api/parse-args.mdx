...
This is a pure parse+coerce function — it never throws for missing required values. Use [`validateParsed()`](/docs/api/validate-parsed) to enforce required constraints separately.

## Signature

```ts
function parseArgs<A extends ArgsDef = ArgsDef, F extends FlagsDef = FlagsDef>(
  command: CommandNode,
  argv: string[],
): ParseResult<A, F>;
```
...
| omitted         | Positional args only: kept as the raw string token (or raw string array for variadic args). Flags must declare `type` because parser grammar determines whether `--flag value` is consumed. |

Raw mode is automatic when a positional arg is defined without a `type`. The
`arg()` helper in [`@crustjs/validate`](/docs/modules/validate) uses this mode
by default; its schema decides requiredness, defaults, and transformations by
validating the raw value (including `undefined` when the arg is missing).
...
const cmd = new Crust("example").args([{ name: "file", type: "string", required: true }]).flags({
