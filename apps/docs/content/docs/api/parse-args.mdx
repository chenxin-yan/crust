...
Raw mode is automatic when a positional arg is defined without a `type`. It
exists for schema-backed validation: the schema decides requiredness, defaults,
and transformations by validating the raw value (including `undefined` when
the arg is missing).
