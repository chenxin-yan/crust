...
Crust supports six built-in flag type literals:
...
.flags({
  ...
  endpoint: { type: "url" },      // --endpoint https://api  (URL instance)
  out:      { type: "path" },     // --out ./dist            (absolute path string)
  config:   { type: "json" },     // --config '{"k":1}'     (parsed JSON, unknown)
})
```
...
- **`"url"`** — Parsed via `new URL()` into a `URL` instance. Throws `PARSE` on values without a protocol
- **`"path"`** — `~` is expanded against `$HOME`, then resolved against `process.cwd()` to an absolute string
- **`"json"`** — Parsed via `JSON.parse()` to `unknown`. Wrap the value in single quotes to survive your shell

See [Built-in Types](/docs/guide/built-in-types) for the deep dive, edge cases, and the `parse` escape hatch for custom transformations.
