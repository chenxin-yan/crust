...
Flags are named options passed with `--` (long form) or `-` short form. They're defined via `.flags()`.
...
Flags accept six built-in `type` literals: `string`, `number`, `boolean`, `url`, `path`, and `json`. See [Types](/docs/guide/types) for the full semantics, error modes, completion behaviour, and the `parse` escape hatch for custom transformations like dates, durations, or ports.
...
<Callout type="warn">
  Alias collisions are caught at **compile time** and **runtime**. If an alias conflicts with
  another flag name or another alias, you'll get an error.
</Callout>

<Callout type="warn">
  The `no-` prefix is reserved for boolean negation. Do not define flag names, short forms, or
  aliases that start with `no-`.
</Callout>
...
```ts
.flags({ config: { type: "string", required: true } })
```
...
`choices` is **validated at parse time** — passing a value outside the list throws `CrustError("PARSE", …)`. When combined with `parse`, the raw argv token is checked against `choices` first; `parse` only runs on values that pass the choices gate. Shell-completion plugins also consume `choices` to emit value candidates after `--flag=` or `--flag <TAB>`. `choices` is only available on string-typed flags; using it on number or boolean flags is a compile-time error. See the [API reference](/docs/api/types#flagdef) for details.
