...
Builder chaining accumulates `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()`; duplicate Extension IDs keep the last registration. TypeScript rejects provable definition and collision errors. Spread dynamic collections with `.flags(...defs)` or `.args(...defs)`. Their actual local and destination constraints are automatically checked, including already-registered Extension flags; invalid definitions throw `DEFINITION` at consumption.
