...
Flags are named options passed with `--` (long form) or `-` (short alias). They're defined as a record via `.flags()`.
...
Each flag is a `FlagDef` object with properties like `type`, `description`, `default`, `required`, `alias`, `multiple`, and `inherit`. See the [FlagDef reference](/docs/api/types#flagdef) for the full property table.
...
<Callout type="warn">
  The `no-` prefix is reserved for boolean negation. Do not define flag names or aliases that start with `no-`.
</Callout>
