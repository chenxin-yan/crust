...
Builder chaining follows the same accumulation rule for `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()` (except duplicate `ExtensionId`s, where `.extend()` keeps only the last registration). Definition correctness is enforced by TypeScript for statically known definitions; inputs the type system cannot see — flags built from config objects, duplicate or misplaced arguments, and dynamic `.add()` name collisions — throw runtime `DEFINITION` errors instead.
