...
<include lang="ts">../../../examples/guide/flags.ts</include>
...
Builder chaining accumulates `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()`; duplicate Extension IDs keep the last registration. TypeScript rejects provable definition and collision errors. Spread dynamic collections with `.flags(...defs)` or `.args(...defs)`. Builder calls check actual local constraints; preparation with `snapshot()`, `run()`, or `execute()` checks destination relations with Extension flags. Invalid definitions throw `DEFINITION` errors.

## Aliases and negation

Use one character for `short`, such as `"o"` for `-o`. `aliases` adds alternate names with two dashes, such as `"out"` for `--out`. A one-character alias such as `"x"` accepts both `--x` and `-x`:
...
Choices are supported on string flags. When `choices` and `default` are literals, TypeScript reports `FIX_DEFAULT_CHOICE` if the default falls outside the set. Defaults against dynamic choices are checked automatically when consumed. Incoming choices are also checked for both argv and structured invocation; an invalid supplied value produces a `PARSE` failure.
