...
Builder chaining follows the same accumulation rule for `.flags()`, `.args()`, `.provide()`, `.add()`, and root-only `.extend()`. Definition correctness is enforced by TypeScript rather than repeated at runtime.
