...
- Shell completion **does** offer file candidates for `path` flags and positional arguments. `path` flags are emitted explicitly per shell (bash `compgen -f`, zsh `_files`, fish `__fish_complete_path`); `path` positional arguments rely on each shell's default filename fallback (bash `complete -o default`, zsh `_files`, fish `__fish_complete_path`).
