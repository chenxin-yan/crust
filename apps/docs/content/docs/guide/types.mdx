...
<Callout type="info">
  `JSON.parse` loses precision on integers above `Number.MAX_SAFE_INTEGER` (2^53 − 1). If you need
  bigints, use `type: "string"` with a custom `parse` — see the [BigInt
  recipe](#bigint--arbitrary-precision-integers).
</Callout>
...
`parse` must be synchronous. Async parsers would return a `Promise` where a value was expected; Crust rejects them at command setup with `CrustError("DEFINITION", …)`:

```ts
.flags({
  endpoint: { type: "string", parse: async (s) => fetch(s) },
})
// → CrustError("DEFINITION", "Async parse not supported for flag --endpoint. …")
```
...
<Callout type="info">
  Defaults are raw strings. Crust runs `parse(String(default))` so the runtime value matches the
  inferred type — don't pre-parse defaults. If a recipe is reused across commands, extract `parse`
  as a plain function and reference it from each definition.
</Callout>
