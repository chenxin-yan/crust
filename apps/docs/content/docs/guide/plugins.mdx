...
| `addSubCommand(parent, name, command)` | Inject a subcommand (silently skipped if user already defined one with the same name) |

#### Injecting Subcommands

Plugins can register subcommands during setup. If the user already defined a subcommand with the same name, the plugin's injection is silently skipped — user definitions always take priority:
