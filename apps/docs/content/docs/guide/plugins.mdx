...
| Action | Description |
| --- | --- |
| `addFlag(command, name, def)` | Inject a flag into a command's flags object (overrides if name exists) |
...
const myPlugin: CrustPlugin = {
  ...
  setup(context, actions) {
    ...
    actions.addSubCommand(context.rootCommand, "status", statusCmd);
  },
};
```
