---
title: Split Commands Across Files
description: Pull typed command dependencies without coupling command files to the application root.
---

For a small application, add definitions inline at the root. When a command becomes large or reusable, export an inert definition from its own file and add it at the composition root.

## Shared definitions

Keep shared Context factories and their owned flag definitions in a dependency-free module:

<include lang="ts" meta='title="src/shared.ts"'>
  ../../../examples/split-files/shared.ts
</include>

Here `logger` owns `--verbose`, so its setup consumes the raw flag and commands depend only on the derived logger value. The flag definition and its consumer stay together.

## Standalone command definition

A command file imports `defineCommand` and the shared values, never the application builder:

<include lang="ts" meta='title="src/commands/greet.ts"'>
  ../../../examples/split-files/commands/greet.ts
</include>

The action pulls typed dependencies from their factories. The exported definition is inert and carries its own name: importing it does not build or run a command.

## Completed root

Define the root and its application-wide Extensions independently:

<include lang="ts" meta='title="src/app.ts"'>
  ../../../examples/split-files/app.ts
</include>

## Add and execute

The composition root imports both sides:

<include lang="ts" meta='title="src/cli.ts"'>
  ../../../examples/split-files/cli.ts
</include>

Because `.provide(logger())` precedes `.add()`, the logger and its owned `--verbose` flag are available on the command path. A missing provider is reported when the action calls `ctx.use(logger)`. [How `.add()` and `.as()` renaming work](/docs/guide/subcommands) — including the finalize-before-adding rule — is covered in Subcommands.

## Inline or split?

Add `defineCommand(name, recipe)` inline when the implementation is short and local. Move the definition to its own file when that improves readability or when the same command should be added more than once; `ctx.use(factory)` keeps its dependencies explicit without extra command metadata.
