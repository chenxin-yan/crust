...
<Callout type="warn">
  `.extend()` is root-only. Extensions apply to the entire application, so attaching one to a child
  is a `DEFINITION` error.
</Callout>

## Author an Extension

`extension(name, config)` returns a frozen plain value:

```ts
import { Crust, extension } from "@crustjs/core";

export const diagnostics = extension("diagnostics", {
  flags: {
    trace: {
      type: "boolean",
...
Extension commands are configured `Crust` builders attached at the root. Their input definitions are validated before an intercept runs.
