...
A `preRun` hook runs before flag validation, so it may only pull Contexts whose full dependency closure owns no flags. Flag-owning Contexts are available from the action and from `postRun` after validation; they remain unavailable after `ctx.finish()`, which skips validation. Values stay live through all `postRun` hooks and are disposed afterwards.
