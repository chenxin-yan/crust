...
Use a build hook when an Extension must generate files alongside the CLI. It receives the frozen Command Snapshot and returns the files to ship, each a path relative to the output directory plus its `string` or `Uint8Array` content. Core validates every path, rejects a path another hook already produced, writes the files, and records exactly what it wrote; the hook itself touches no disk.
