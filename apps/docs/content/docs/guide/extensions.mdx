...
`defineExtension(name, config)` returns a frozen plain value. The config can contain `flags`, `commands`, `sections`, `metadata`, and `hooks`:
...
Commands can declare plain-text documentation sections in their metadata. Help renders them after Options, and man-page generation emits one mdoc section per root-level entry (subcommand sections appear in help only):
...
Extensions can append sections to any command while the application prepares:
...
Optional `metadata` carries JSON-serializable Extension data for build and other tooling. The root Command Snapshot exposes every loaded Extension under `extensions`; metadata entries and the list are deeply frozen. Subcommand snapshots do not repeat this root-only field.
