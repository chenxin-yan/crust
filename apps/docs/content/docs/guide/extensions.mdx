...
Official Extension identities use the `crust:` namespace, which is reserved for official Crust identities. Third-party packages should namespace published Extension ids with their own prefix; application-local ids can remain plain.
...
A renderer exports its own consumer id and selects its sections with `sectionsFor()`. The official `HELP`, `MAN`, and `SKILLS` constants resolve to reserved `crust:*` ids; use those exports rather than raw strings. Core does not maintain a fixed list of consumers.
