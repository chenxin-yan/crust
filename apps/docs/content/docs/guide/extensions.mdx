...
`defineExtension(name, config)` returns a frozen plain value. The config can contain `flags`, `commands`, `commandSections`, `build`, and `hooks`:
...
Extensions can append sections to any command while the application prepares. The `commandSections` field is a contribution point into the target commands' `meta.sections`, not documentation of the Extension itself:
...
`build(ctx)` generates artifacts when invoked by build tooling such as `crust build`; it does not run during `run()` or `execute()`. The context contains the frozen root Command Snapshot and a resolved absolute `outDir`. Build hooks may be async, and thrown or rejected errors propagate to the build tool.
