...
Arguments are positional values passed to your CLI without a flag name. They're defined via `.args()`, and Crust maps them to named, typed values.
...
Positional arguments accept the same six built-in `type` literals as flags: `string`, `number`, `boolean`, `url`, `path`, and `json`. See [Types](/docs/guide/types) for the full semantics, error modes, and the `parse` escape hatch for custom transformations.
...
<Callout type="warn">
  Only the **last** argument can be variadic. Crust enforces this at both compile time and runtime.
  Placing `variadic: true` on a non-last argument produces a type error.
</Callout>
