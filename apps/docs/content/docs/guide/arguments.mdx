...
`.args(...defs)` maps positional tokens to names in the order the definitions are passed. Repeated `.args()` calls append in call order. For TypeScript consumers, duplicate names within or across calls fail typecheck with `FIX_DUPLICATE_ARG`; the same misuse throws a `DEFINITION` error at runtime for plain-JS consumers or dynamically built definitions. Pass values from `defineArg(name, def)` or inline literals:
