...
Crust supports six built-in argument type literals:
...
.args([
  ...
  { name: "target",  type: "url" },      // Parsed to a URL instance
  { name: "out",     type: "path" },     // Absolute path string
  { name: "config",  type: "json" },     // Parsed JSON (unknown)
])
```
...
- **`"url"`** — Parsed via `new URL()` into a `URL` instance
- **`"path"`** — `~` is expanded, then resolved against `process.cwd()` to an absolute string
- **`"json"`** — Parsed via `JSON.parse()` to `unknown`

See [Built-in Types](/docs/guide/built-in-types) for the deep dive and the `parse` escape hatch.
