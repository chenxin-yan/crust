...
The same check runs during `prepareCommandTree()` for plugin-installed subcommands. Plugin `addSubCommand` calls that collide are skipped with a warning, matching the behavior for duplicate plugin subcommand names.

Ancestor, descendant, and cousin commands may reuse the same token by default because routing resolves one level at a time. For example, `my-cli issue` and `my-cli project issue` are different command namespaces. If you want a large CLI to avoid that reuse for readability, enable opt-in diagnostics:

```ts
const { warnings } = await app.prepareCommandTree({ aliasDiagnostics: "warn" });
```

`crust build --alias-diagnostics=warn` reports the same diagnostics during pre-compile validation, and `strict` turns those diagnostics into `DEFINITION` errors.
