...
An unknown subcommand produces `CrustError` code `COMMAND_NOT_FOUND`. Its details include the input, visible available commands, the command path, and a readonly snapshot of the parent command. Hidden commands are omitted from `available`. The official `didYouMean()` Extension uses this information to suggest nearby names.
