...
All three of the following invoke the same command:

```sh
my-cli issue list
my-cli issues list   # via alias "issues"
my-cli i list        # via alias "i"
```

The canonical name is what appears in `commandPath`, error messages, and `didYouMeanPlugin` suggestions — the alias the user typed is not preserved. Help renders the canonical name with aliases inline:

```text
COMMANDS:
  issue (issues, i)   Manage issues
```

### Conflict policy

Alias strings share a namespace with the command's **direct siblings**. An alias must not collide with the command's own canonical `name`, with any sibling's `name`, or with any sibling's alias. Aliases must also be non-empty, contain no whitespace, and not start with `-`. Collisions throw a `CrustError("DEFINITION", …)` at registration time:
...
Ancestor and descendant command tokens are checked too, both at runtime and in TypeScript for statically-authored command trees:

```ts
new Crust("my-cli").command("issue", (issue) =>
  // ❌ TypeScript error + runtime validation error:
  // descendant command name "issue" collides with ancestor command name "issue"
  issue.command("issue", (cmd) => cmd.run(() => {})),
);
```

Commands in separate branches may reuse the same token because routing resolves one level at a time. For example, `my-cli issue list` and `my-cli project list` are different command namespaces.

The same runtime check runs during `prepareCommandTree()` for plugin-installed subcommands. Plugin `addSubCommand` calls that collide with siblings are skipped with a warning, matching the behavior for duplicate plugin subcommand names.
...
`hidden` affects user-facing listings only — the command remains directly invocable by name and alias resolution still works, but first-party renderers and `didYouMeanPlugin` omit it from suggestions and available-command lists. It is intended for internal/runtime commands like the `__complete` entrypoint used by shell-completion plugins. See the [`CommandMeta` reference](/docs/api/types#commandmeta) for details.
...
await app.use(helpPlugin()).command(deployCmd).execute();
```
...
```ts
{
  ...
  available: ["build", "dev"],          // Canonical sibling subcommands
  commandPath: ["my-cli"],              // Path to the parent
  parentCommand: main,                  // The parent command node
}
```

`available` contains canonical sibling names only. Alias-aware tooling can read `parentCommand.subCommands` from the error details to inspect each sibling's `meta.aliases`. `didYouMeanPlugin` does that so it can match alias spellings while still reporting the canonical name as its suggestion.
