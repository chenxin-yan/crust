...
| ----------------------------------- | ------------------------------------------- | ---------------------------------------------- |
| `.command(name, cb)`                | The command tree is defined inline          | Golden path with the strongest inference       |
| `app.sub(name)` + `.command(child)` | The subcommand lives in another source file | Parent-typed split module with inherited flags |
...
const main = new Crust("my-cli").extend(help()).command("issue", (cmd) =>
...
The canonical name is what appears in `commandPath`, error messages, and `didYouMean()` suggestions — the alias the user typed is not preserved. Help renders the canonical name with aliases inline:
...
`hidden` affects help rendering only — the command remains directly invocable by name, alias resolution still works, and `didYouMean()` still suggests it. It is intended for internal/runtime commands like the `__complete` entrypoint used by shell-completion extensions. See the [`CommandMeta` reference](/docs/api/types#commandmeta) for details.
...
Use `.sub()` when the subcommand lives in another file and must retain the parent app's inherited flags. Keep the base app in a file that imports no command modules; import both the app and commands from the entrypoint.
...
await app.extend(help()).command(deployCmd).execute();
```

`.sub()` creates a child builder pre-typed with the parent's inheritable flags. Prefer inline `.command(name, cb)` for most CLIs; use `.sub()` only when splitting command definition files without duplicating handler types.
