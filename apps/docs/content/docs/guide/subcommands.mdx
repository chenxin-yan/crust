...
Add subcommands using the `.command()` method:
...
const main = new Crust("my-cli")
  ...
  .command("dev", (cmd) =>
    cmd
      ...
      .flags({ port: { type: "number", default: 3000, alias: "p" } })
      .run(({ flags }) => {
...
For large CLIs, you can define subcommands in separate files using `.sub()`:

```ts title="shared.ts"
import { Crust } from "@crustjs/core";

export const app = new Crust("my-cli")
  .flags({ verbose: { type: "boolean", alias: "v", inherit: true } });
```
...
The `.sub()` method creates a child builder pre-typed with the parent's inheritable flags. This enables full TypeScript inference in the subcommand file without needing `Crust<any, any, any>`. See the [`.sub()` reference](/docs/api/crust#subname) for details.
