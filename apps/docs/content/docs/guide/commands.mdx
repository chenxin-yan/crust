---
title: Commands
description: Define immutable, typed commands and Command Actions.
---

import { Callout } from "fumadocs-ui/components/callout";

A command combines metadata, inputs, dependencies, and one Command Action.

```ts
import { Crust } from "@crustjs/core";

const app = new Crust("greet", { description: "Print a greeting" })
  .args({ name: "name", type: "string", required: true })
  .flags({ name: "loud", type: "boolean", short: "l" })
  .action(({ args, flags, stdout }) => {
    const text = `Hello, ${args.name}!`;
    stdout(flags.loud ? text.toUpperCase() : text);
  });

await app.execute();
```

## Immutable builders

Every builder method returns a new `Crust` value. Keep the returned value:

```ts
const base = new Crust("app", { description: "My app" });
const withFlags = base.flags({ name: "verbose", type: "boolean" });
```

Static root metadata belongs in the constructor. Subcommand metadata belongs in `defineCommand()` config:

```ts
import { defineCommand } from "@crustjs/core";

const greet = defineCommand(
  "greet",
  { description: "Print a greeting", aliases: ["hello"] },
  (command) => command.action(() => {}),
);
```

Fluent methods remain immutable, permitting safe composition while preserving input inference.

## Command Action context

`.action()` receives a typed `CrustCommandContext`:

<auto-type-table
  path="../../../../../packages/core/src/command/crust.ts"
  name="CrustCommandContext"
/>

```ts
const command = new Crust("inspect").action(({ command, stdout }) => {
  stdout(command.meta.name);
});
```

Throw ordinary errors from Command Actions. Crust does not wrap them.

## Invocation

Use `execute()` at the terminal boundary:

```ts
await app.execute();
```

Use `run()` from tests or another program:

```ts
const output: string[] = [];
const diagnostics: string[] = [];

await app.run(
  [],
  { args: { name: "Ada" } },
  {
    stdout: (line) => output.push(line),
    stderr: (line) => diagnostics.push(line),
  },
);
```

`run()` resolves to a status-discriminated outcome: `completed` carries the action result, while `finished` names the Extension whose `preRun` hook ended the invocation. It throws failures to the caller, including [combined invocation and disposal failures](/docs/guide/lifecycle#error-flow). `execute()` presents errors through Extension `onError` hooks, sets `process.exitCode`, and resolves. See [Execution Model](/docs/guide/lifecycle).

A parent with a Command Action treats unmatched argv command tokens as positional arguments. Typed `run()` rejects unknown path segments. See [Unknown commands](/docs/guide/subcommands#unknown-commands).

<Callout type="info">
  A command without a Command Action is valid when it owns subcommands. Invoking it directly
  resolves to `{ status: "completed", result: undefined }`; register the `help()` Extension to
  present the container's help instead.
</Callout>
