...
<Callout>
  Plugins can still inject flags during the setup phase via the `addFlag` action. This is the only
  sanctioned way to modify a command's flags after creation.
</Callout>
