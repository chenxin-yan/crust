...
- only `PUBLIC_*` keys are eligible for build-time constant injection
...
<Callout type="warn">
  `PUBLIC_*` values are embedded in the binary and are therefore public. Never put secrets behind a
  `PUBLIC_*` prefix.
</Callout>
