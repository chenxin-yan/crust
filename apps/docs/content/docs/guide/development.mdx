...
bun run package
cd dist/npm/root
ROOT_TGZ="$(pwd)/$(bun pm pack --quiet)"

cd ../darwin-arm64
PLATFORM_TGZ="$(pwd)/$(bun pm pack --quiet)"
```

Install both tarballs into a throwaway project:
...
```json title="package.json"
{
  ...
  "private": true,
  "dependencies": {
    "<root-package-name>": "file:/absolute/path/to/root.tgz",
    "<platform-package-name>": "file:/absolute/path/to/platform.tgz"
  }
}
```
