...
| Check | Error Message |
| --- | --- |
| Empty `meta.name` | `meta.name is required and must be a non-empty string` |
| Flag name starts with `no-` | `Flag name "--no-cache" must not start with "no-"; define "cache" instead...` |
| Alias starts with `no-` | `Alias "--no-store" on flag "--cache" must not start with "no-"...` |
...
| `no-` prefixed flag name (defense-in-depth) | `DEFINITION` | `Flag name "--no-cache" must not start with "no-"...` |
| `no-` prefixed alias (defense-in-depth) | `DEFINITION` | `Alias "--no-store" on flag "--cache" must not start with "no-"...` |
