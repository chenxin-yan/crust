...
`npm publish` runs inside each `.crust/<package>` directory, so a project-level `.npmrc` is not read. Authenticate with [trusted publishing](https://docs.npmjs.com/trusted-publishers), point `NPM_CONFIG_USERCONFIG` at an `.npmrc` you write in the workflow, or use `~/.npmrc`; `--registry` overrides the destination. The existence lookup replays the staged `publishConfig` registry settings (`registry`, `@scope:registry`, `scope`) to `npm view` and follows npm 11's precedence, where CLI flags win over `publishConfig`; older npm resolves `publishConfig` first, so use npm 11 (which trusted publishing needs anyway). npm reads `publishConfig.access` from each staged `package.json`, so a scoped public package needs `"publishConfig": { "access": "public" }` in its `package.json`, which `crust build` copies into every staged package.
