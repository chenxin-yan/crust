...
1. **Prepare definitions** — apply Extensions and reject command, flag, and alias collisions (duplicate Context names are rejected earlier, at definition time).
...
7. **Construct Contexts** — create Context values for the resolved path in registration order.
...
## Replacing setup and cleanup hooks

The 0.1 `.preRun()` and `.postRun()` APIs are removed:
