...
4. **Run `preRun` hooks** — invoke Extension hooks in prepared order. Hooks may lazily pull flagless Contexts. A hook can return `ctx.finish()` to skip the remaining hooks, validation, schemas, and the Command Action.
...
9. **Run `postRun` hooks** — invoke every Extension hook in reverse prepared order with the completed, finished, or failed outcome. Hooks may pull or reuse Contexts here.
