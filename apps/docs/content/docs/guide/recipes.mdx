...
Two things the recipes above leave implicit:

- **Defaults are raw strings.** Crust runs `parse(String(default))` so the runtime value matches `T` — don't pre-parse them.
- **Reuse across commands.** If the same recipe appears in multiple commands, extract `parse` as a plain function and reference it from each flag definition.
