...
const app = new Crust("my-cli")
  ...
  .action(async ({ ctx, stdout }) => stdout((await ctx.use(database)).query("select 1")));
```
...
Use `captureExecute(app, argv)` for raw parser behavior such as short-flag bundles, `--flag=value`, invalid spellings, and Extension-contributed commands. Those commands materialize at runtime and cannot appear in the typed `captureRun()` or `runInteractive()` path.
