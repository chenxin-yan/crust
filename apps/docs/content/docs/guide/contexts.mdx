...
A ctx dependency that is not provided on the path is a `CrustError` with code `DEFINITION`, raised at dispatch and also caught by [`crust build`](/docs/guide/build)'s command-tree validation. For TypeScript consumers, a dependency cycle whose members are provided with literal types fails typecheck with `FIX_CONTEXT_CYCLE`; cycles through inherited parent Contexts and dynamically built graphs remain runtime-only. Runtime validation reports every cycle as `DEFINITION`.
