...
A missing dependency or dependency cycle is a `CrustError` with code `DEFINITION`, raised by `.provide()` during runtime normalization. Cycles are enforced only at runtime; TypeScript brands do not attempt recursive cycle detection.
