...
| `--stage-dir` | — | `"string"` | `dist/npm` | Directory to stage npm packages when using `--distribute` |
...
## Per-OS Distribution with `--distribute`

For distributing your CLI via npm with platform-specific packages, use the `--distribute` flag. This creates a root package with optional dependencies on per-platform packages, enabling npm to install only the binary for the user's platform.
...
crust build --distribute --target linux-x64 --target darwin-arm64

# Use a custom staging directory
crust build --distribute --stage-dir ./npm-packages
```

### What `--distribute` Creates

When using `--distribute`, instead of raw binaries, Crust creates:
...
├── my-cli/                    # Root package (resolver + metadata)
│   ├── package.json
│   └── bin/
│       └── my-cli             # Shell resolver
├── my-cli-linux-x64/          # Linux x64 package
│   ├── package.json
│   └── bin/
│       └── my-cli
├── my-cli-linux-arm64/        # Linux ARM64 package
│   ├── package.json
│   └── bin/
│       └── my-cli
├── my-cli-darwin-x64/         # macOS Intel package
│   ├── package.json
│   └── bin/
│       └── my-cli
├── my-cli-darwin-arm64/       # macOS Apple Silicon package
│   ├── package.json
│   └── bin/
│       └── my-cli
├── my-cli-windows-x64/        # Windows x64 package
│   ├── package.json
│   └── bin/
│       └── my-cli.exe
└── my-cli-windows-arm64/      # Windows ARM64 package
    ├── package.json
    └── bin/
        └── my-cli.exe
```
...
After staging with `--distribute`, use `crust publish` to publish all packages in the correct order:
...
```json title="package.json"
{
  ...
  "scripts": {
    "build": "crust build --distribute",
    "prepack": "bun run build"
  }
}
```

After building, publish all platform packages:

```sh
# First time: publish all packages
crust publish

# Subsequent publishes: same command publishes in correct order
```
