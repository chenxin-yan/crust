...
The CLI flag overrides project configuration. Existing projects without `crust.runtime` continue to use Bun. Bun compilation prefers a `bun` executable on `PATH` and otherwise uses the runtime embedded in the packaged Crust executable. Deno builds require `deno` on `PATH`. Node builds use Crust's embedded Bun bundler, so users do not need Bun installed. The packaged `crust` command itself uses a Node launcher, so Node is required to start the build tool.

## Basic Usage

By default, `crust build` reads `src/cli.ts` and writes build artifacts to `dist`.
...
| Runtime | Compiler                  | Output                                       | Additional requirements        |
| ------- | ------------------------- | -------------------------------------------- | ------------------------------ |
...
Use canonical compiler names with `--target`, such as `bun-darwin-arm64` for Bun or `aarch64-apple-darwin` for Deno on Apple Silicon. Repeat `--target` to select multiple platforms, or omit it to build for all supported platforms of the selected runtime.

For the complete supported target lists, see `BUN_TARGET_NAMES` and `DENO_TARGET_NAMES` in the [build target definitions](https://github.com/chenxin-yan/crust/blob/main/packages/crust/src/utils/build-helpers.ts). These track `main`; `crust build --help` describes the option but does not list targets.
...
By default, `crust build` materializes your full command tree before compiling. It evaluates the entry under Bun in a subprocess to prepare a Command Snapshot, forcing all command recipes and Extensions. This happens for every target runtime, including Node and Deno; packaged Crust uses its embedded Bun. Target-only APIs used during preparation can fail under Bun. TypeScript reports statically representable definition errors during compilation. Runtime materialization still validates dynamic or config-built definitions, materialized command recipes, and authored and Extension-contributed documentation sections.

In this subprocess, `.execute()` materializes the full command graph, including nested subcommands and Extension-owned flags and commands, validates documentation sections, runs Extension build hooks, writes the final JSON snapshot, and exits without executing Command Actions or code after `await app.execute()`.
...
The subprocess has a 30-second timeout covering entry evaluation and build hooks. A timeout fails the build.

After preparing the initial snapshot, the same subprocess runs every registered Extension `build` hook in registration order. After each hook, Crust re-evaluates Extension section callbacks on a fresh copy of the materialized tree — command recipes still run once — so each later hook and the final snapshot observe outputs from earlier hooks. A hook receives the snapshot prepared before it starts, so it cannot observe its own outputs through that initial `ctx.snapshot`; register a later renderer hook when an artifact must include those outputs. Extension presence is the source of intent: for example, `man()` writes a manual and `skill()` writes packaged skills without Extension-specific build flags. Hook errors fail the build with the Extension id.

Hooks may return the files they wrote as paths relative to `outDir`. `crust build` reports these artifacts by Extension id before compilation; hooks that return nothing are shown as having run without an artifact report. Reported paths are normalized to POSIX-style separators. Absolute paths and paths that escape `outDir` fail the build and identify the responsible Extension. The temporary Build Report is used only to communicate with the parent build process and is not written to `dist/`. Each build hook must own and clean a unique top-level directory in the artifact output directory. Top-level artifact directories are retained under `<outdir>` — or beside the binary when `--outfile` relocates it — and when `--package` is used they are staged into the root package and beside each platform binary, so executable-relative source resolution finds them at runtime. `--stage-dir` must not contain `--outdir`; staging replaces the stage directory and would delete the artifacts. The artifact name `bin` is reserved for the generated npm executables.

Raw binaries do not embed these artifact directories. Include them beside the binary when distributing an archive. The generated binary template's `files` list includes only the resolver scripts and Bun binaries, not `man`, `skills`, or `completions`. Use `--package` for npm distribution with those directories, or include them explicitly in your own package's `files` list.
...
### Skipping snapshot preparation

Snapshot preparation is enabled by default. To skip it:

```sh
crust build --no-validate
```

Use this escape hatch for entries with unavoidable import-time side effects. It can also help when the snapshot subprocess cannot resolve your project's dependencies in a particular environment (for example, a non-standard module resolution setup), or when the entry never calls `await app.execute()` — such entries fail with a missing-snapshot error. Skipping preparation removes its runtime definition and documentation validation and every Extension build hook. With `--package`, no artifact directories are staged, even if they already exist.
...
- A project `package.json` with `name` and `version`.
- If `bin` is an object, it must contain exactly one entry. A string `bin` is supported; if omitted, Crust infers the command name.
- No `--outfile`. Use `--stage-dir` to choose the staging destination.
...
2. **Per-platform packages** — One package per target containing that platform's binary and any staged Extension artifact directories
...
The root package exposes `bin/my-cli.js` as its `bin` entry. This is a small Node launcher, so end users need Node to start it; the platform binaries embed Bun. npm generates platform launchers from that entry during install.
...
The publish command reads `manifest.json`, verifies staged directories and metadata, and publishes platform packages first, then the root package. It runs Bun's `publish` command with `--access public --no-git-checks` by default and stops on the first failed publish. `--access`, `--tag`, and `--registry` override the corresponding publish options. `--no-verify` skips staged metadata verification. `--dry-run` prints the order and commands without publishing; verification still runs unless disabled.

For the complete publishing option reference, run:
...
Per-platform packages are named by appending the platform suffix to your package name:
