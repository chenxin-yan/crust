...
2. **A JS resolver script** — `dist/cli.js` (customizable via `--resolver`) that detects the host platform and spawns the correct binary
...
```
dist/
├── cli.js                               # JS resolver (entry point)
...
The JS resolver (`dist/cli.js` by default) is a `#!/usr/bin/env node` script that:
