...
The mechanical columns below are parity-checked against the command definitions. “Declared default” is only a definition default; `—` means none is declared. Effective defaults are resolved later: runtime uses the flag, then `package.json`, then Bun; name is [auto-detected](#binary-naming); omitted targets select all platforms for Bun/Deno; minification defaults to enabled for Bun/Node and is unavailable for Deno.

| Flag spellings                | Type                  | Choices               | Declared default | Description                                                                         |
| ----------------------------- | --------------------- | --------------------- | ---------------- | ----------------------------------------------------------------------------------- |
| `-e`, `--entry`               | `string`              | —                     | `"src/cli.ts"`   | Entry file path                                                                     |
| `-o`, `--outfile`             | `string`              | —                     | —                | Output file path (single-target only)                                               |
| `-n`, `--name`                | `string`              | —                     | —                | Binary name                                                                         |
| `--minify`, `--no-minify`     | `boolean`             | —                     | —                | Minify Bun and Node output; not applicable to Deno (`deno compile` has no minifier) |
| `--runtime`                   | `string`              | `bun`, `deno`, `node` | —                | `bun`, `deno`, or `node`; overrides `package.json`                                  |
| `-t`, `--target`              | `string` (repeatable) | —                     | —                | Canonical Bun or Deno compiler target; unsupported for Node                         |
| `-d`, `--outdir`              | `string`              | —                     | `"dist"`         | Output directory for compiled binaries                                              |
| `-r`, `--resolver`            | `string`              | —                     | `"cli"`          | Resolver script filename (multi-target only, no extension)                          |
...
| `--package`, `--no-package`   | `boolean`             | —                     | `false`          | Stage npm packages in `dist/npm` instead of raw binaries                            |
...
| `--dry-run`, `--no-dry-run` | `boolean` | —       | `false`          | Print publish order and commands without publishing      |
...
Per-platform packages are named by appending the platform suffix (see [Supported Bun targets](#supported-bun-targets)) to your package name:
