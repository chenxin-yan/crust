...
The `crust build` command compiles your CLI entry file into a standalone Bun or Deno executable. Bun builds run `bun build --compile`; Deno builds run `deno compile`. Multi-target builds generate a platform-detecting shell resolver, so no runtime is required on the end user's machine.
...
```sh
# Build for all platforms (Bun default; detected Deno projects build the host)
crust build

# Build with a custom entry point
crust build --entry src/main.ts

# Select a runtime and build for the host
crust build --target deno
...
| `--minify`                     | —     | `"boolean"`             | `true`        | Minify Bun output (Deno controls its compiled representation)            |
| `--target`                     | `-t`  | `"string"` (repeatable) | auto-detected | Runtime (`bun` or `deno`) and optional canonical compile target(s)       |
...
| `--validate` / `--no-validate` | —     | `"boolean"`             | `true`        | Bun: materialize commands and run Extension build hooks before compiling |
...
An explicit `--target bun` or `--target deno` selects the compiler and builds for the host. Add canonical targets by repeating the flag, for example:

```sh
crust build --target deno --target x86_64-unknown-linux-gnu --target aarch64-apple-darwin
```

Without an explicit target, Crust detects the runtime in this order:

1. `package.json` `engines.deno`, then `engines.bun`
2. Deno markers (`deno.json`, `deno.jsonc`, or `deno.lock`), then Bun markers (`bun.lock` or `bun.lockb`)
3. Bun as the fallback

Deno wins when both runtimes are declared or both marker families exist. A detected Deno project builds for the host. A detected or fallback Bun project keeps the existing default of building every supported Bun target. Bun and Deno target strings cannot be mixed in one build.

Deno binaries are compiled with `-A` (all permissions): Crust reads environment variables at startup, so a sandboxed binary would fail before reaching your command. This matches Bun executables, which have no permission sandbox.
...
Deno supports `x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`, `x86_64-apple-darwin`, `aarch64-apple-darwin`, and `x86_64-pc-windows-msvc`.

## Multi-Target Build (Bun Default)

When Bun is detected and no `--target` is specified (or multiple targets are given), `crust build` produces:
...
When a runtime name or exactly one canonical target is specified, `crust build` produces a single binary without a resolver:
...
Bun builds embed only `PUBLIC_*` values as build-time constants; `--env-file` (repeatable, also with `--package`) selects explicit env files. Deno builds pass those files to the compiler process but do not embed `PUBLIC_*` values.
...
<Callout type="warn">
  Deno builds currently skip Command Snapshot validation and Extension build hooks. Build with Bun
  when the CLI requires generated manuals, skills, or other Extension artifacts.
</Callout>
...
For distributing your Bun-compiled CLI via npm with platform-specific packages, use the `--package` flag. This creates a root package with optional dependencies on per-platform packages, enabling npm to install only the binary for the user's platform. Deno builds do not support `--package` yet.
