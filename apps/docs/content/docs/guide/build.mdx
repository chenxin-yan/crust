...
The `crust build` command compiles your CLI entry file into standalone Bun executables using `bun build --compile`. It supports cross-platform compilation and generates a platform-detecting shell resolver for multi-target distribution — no runtime (Node.js or Bun) required on the end user's machine.
...
The resolver (`dist/cli` by default) is a `#!/bin/sh` POSIX shell script that:
