...
If you need public build-time constants, `crust build` can use Bun's cwd env by default or take explicit `--env-file` inputs. See [Environment Variables](/docs/guide/environment) for the full env model.
...
See [Per-OS Distribution](/docs/guide/build#per-os-distribution-with---package) for details.
