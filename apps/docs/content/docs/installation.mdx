...
To publish via npm with automatic platform detection, add the `distribute` and `publish` scripts:
...
The `distribute` script creates platform-specific packages in `dist/npm/`. The `publish` script publishes them in manifest order. See [Per-OS Distribution](/docs/guide/build#per-os-distribution-with-distribute) for details.
