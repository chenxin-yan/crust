...
<Callout>
  If you're starting a new project, the easiest way is `bun create crust my-cli`. See the [Quick
...
const main = new Crust("my-cli").meta({ description: "My CLI tool" }).run(() => {
...
export const app = new Crust("my-cli").meta({ description: "My CLI tool" }).flags({
  greet: {
    type: "string",
...
The `package` script creates platform-specific packages in `dist/npm/`. The `publish` script publishes them in manifest order. See [Per-OS Distribution](/docs/guide/build#per-os-distribution-with---package) for details.

If you need public build-time constants, `crust build` can read `PUBLIC_*` values from Bun's cwd env by default, or you can pass `--env-file` for explicit env files. See [Environment Variables](/docs/guide/environment) for the full runtime-vs-build-time model.
